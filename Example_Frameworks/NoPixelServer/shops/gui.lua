--[[
    -- Type: Module
    -- Name: gui
    -- Use: Provides a lightweight menu utility
    -- Created: 2024-06-01
    -- By: VSSVSSN
--]]
...
function Menu.updateSelection()
    ...
    for i = 1, #Menu.GUI do
        Menu.GUI[i].active = (i - 1 == Menu.selection)
    end
end
...
function Menu.renderBox(xMin, xMax, yMin, yMax, r, g, b, a)
...
function Menu:drawButtons(options)
    ...
    for _, settings in pairs(Menu.GUI) do
        SetTextFont(0)
        SetTextProportional(0)
        SetTextScale(options.scale, options.scale)
        ...
        DrawText(options.x - options.width / 2 + 0.005, y - 0.02)
        y = y + 0.04
    end
end
