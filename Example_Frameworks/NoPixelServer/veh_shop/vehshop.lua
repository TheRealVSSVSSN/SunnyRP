			...
			if rank > 0 then
				if financedPlates[addplate] ~= nil then
					DrawText3D(
						...
						carspawns[i]["y"],
						carspawns[i]["z"],
						"$" .. math.ceil(price) .. " | Com: %" .. commission .. " | [E] to change | [G] to buy | [F] to Finance "
					)
				else
					DrawText3D(
						carspawns[i]["x"],
						carspawns[i]["y"],
						carspawns[i]["z"],
						"$" .. math.ceil(price) .. " | Com: %" .. commission .. " | [E] to change | [G] to buy. "
					)
				end
			else
				if financedPlates[addplate] ~= nil then
					DrawText3D(
						...
						"$" ..
							math.ceil(price) ..
								" [G] to buy | $" ..
									math.ceil(price / 4) .. " upfront, $" .. math.ceil(price) .. " over 10 weeks, [F] to finance. "
					)
				else
...
function SpawnSaleVehicles()
	...
	for i = 1, #carTable do
		...
		if model == nil then
			print("invalid car model in PDM sale vehicles")
			break
		end
		...
		if carspawns[i] == nil then
			print("invalid car in PDM sale vehicles")
			break
		end

		local veh =
			CreateVehicle(model, carspawns[i]["x"], carspawns[i]["y"], carspawns[i]["z"] - 1, carspawns[i]["h"], false, false)
		SetModelAsNoLongerNeeded(model)
		SetVehicleOnGroundProperly(veh)
				...
				while #vehshop_blips > 0 do
					...
					for i, b in ipairs(vehshop_blips) do
						if insideVehShop then
							...
							if
								vehshop.opened == false and IsPedInAnyVehicle(LocalPed(), true) == false and
									#(vector3(b.pos.entering[1], b.pos.entering[2], b.pos.entering[3]) - GetEntityCoords(LocalPed())) <= 1
							 then
							...
							if vehshop.opened == true then
								DisplayHelpText(
									"~g~" ..
										Controlkey["generalUse"][2] ..
											"~s~ or ~g~" .. Controlkey["generalUseSecondary"][2] .. "~s~ Accepts ~g~Arrows~s~ Move ~g~Backspace~s~ Exit"
								)
							end
...
function ButtonSelected(button)
	...
	elseif
		this == "jobvehicles" or this == "compacts" or this == "coupes" or this == "sedans" or this == "sports" or
			this == "sportsclassics" or
			this == "super" or
			this == "muscle" or
			this == "offroad" or
			this == "suvs" or
...
function Back()
	...
	if vehshop.currentmenu == "main" then
		CloseCreator()
	elseif
		vehshop.currentmenu == "jobvehicles" or vehshop.currentmenu == "compacts" or vehshop.currentmenu == "coupes" or
			vehshop.currentmenu == "sedans" or
			vehshop.currentmenu == "sports" or
			vehshop.currentmenu == "sportsclassics" or
			vehshop.currentmenu == "super" or
			vehshop.currentmenu == "muscle" or
			vehshop.currentmenu == "offroad" or
			vehshop.currentmenu == "suvs" or
			vehshop.currentmenu == "vans" or
			vehshop.currentmenu == "industrial" or
			vehshop.currentmenu == "cycles" or
			vehshop.currentmenu == "motorcycles"
	 then
...
--[[Citizen]] function Initialize(scaleform, veh, vehname)
...
Citizen.CreateThread(
	function()
		while true do
			Citizen.Wait(0)
			if
				(IsControlJustPressed(1, Controlkey["generalUse"][1]) or
					IsControlJustPressed(1, Controlkey["generalUseSecondary"][1])) and
					IsPlayerInRangeOfVehshop()
			 then
				if vehshop.opened then
					CloseCreator()
				else
				...
				for i, button in pairs(menu.buttons) do
					...
					if rank >= br and i >= vehshop.menu.from and i <= vehshop.menu.to then
						...
						if
							selected and
								(IsControlJustPressed(1, Controlkey["generalUse"][1]) or
									IsControlJustPressed(1, Controlkey["generalUseSecondary"][1]))
						 then
...
AddEventHandler(
	"FinishMoneyCheckForVeh",
	function(name, vehicle, price, financed)
		local name = name
