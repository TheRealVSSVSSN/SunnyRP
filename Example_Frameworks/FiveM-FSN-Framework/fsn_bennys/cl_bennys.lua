...
function BennysMenu:onButtonSelected(name, button)
        if button.price then
                if exports["fsn_main"]:fsn_CanAfford(button.price) then
                        ...
                        exports['mythic_notify']:DoHudText('error', 'You cannot afford: $'..tostring(button.price))
                end
        end
end
...
Citizen.CreateThread(function()
        while true do
                ...
                if #(coords - Config.XYZ) < 10.0 then
                        wait = 0
				...
				if lbl ~= nil then
					...
					if mname ~= "NULL" then
                        ...
                        for n, v in pairs(Config.prices.mods[mod]) do
                                local btn = m:addPurchase(v.name, v.price)
                                btn.modtype = mod
                                btn.mod = v.mod
                        end
                end
...
function EnterGarage()
        ...
        if not IsPedInAnyVehicle(ped, false) then
                exports['mythic_notify']:DoHudText('error', 'You need to be driving a vehicle to use this garage.')
                return
        end
        ...
        local damage = (1000 - GetVehicleBodyHealth(veh))/100
