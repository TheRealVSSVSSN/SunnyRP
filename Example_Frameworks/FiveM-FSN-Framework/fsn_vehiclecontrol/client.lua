...
CreateThread(function()
        while true do
                ...
                if IsPedInAnyVehicle(ped, false) then
                        if GetPedInVehicleSeat(GetVehiclePedIsIn(ped, false), 0) == ped and GetIsTaskActive(ped, 165) then
                ...
                if IsPedInAnyVehicle(ped, false) then
                        ...
                        if IsControlPressed(0, 21) and IsControlJustPressed(0, 33) then
                                if seatbelt then
                        ...
                        if IsControlJustPressed(0, 73) and GetPedInVehicleSeat(GetVehiclePedIsIn(ped, false), -1) == ped then
                                if cruise then
                                ...
                                elseif math.floor(GetEntitySpeed(GetVehiclePedIsUsing(ped, true)) * 2.236936) >= 30 then
                        ...
                        if cruise then cruise = false end
                        if notified then notified = false end
                end
        end
end)
------------------------------------------------------------------------------
                ...
                if car ~= 0 and (wasInCar or IsCar(car)) and not exports["fsn_police"]:fsn_PDDuty() then
                        ...
                        if speedBuffer[2] ~= nil
                           ...
                           and (speedBuffer[2] - speedBuffer[1]) > (speedBuffer[1] * 0.255) then
                                ...
                                if IsPointOnRoad(GetEntityCoords(ped), GetVehiclePedIsIn(ped, false)) then
                                        local pos = GetEntityCoords(ped)
                                        TriggerServerEvent('fsn_police:dispatch', {x = pos.x, y = pos.y, z = pos.z}, 17)
                                end
