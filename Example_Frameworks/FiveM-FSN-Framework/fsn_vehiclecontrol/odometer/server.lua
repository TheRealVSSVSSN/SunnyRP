...
AddEventHandler('fsn_odometer:addMileage', function(plate, mileage)
    MySQL.ready(function()
        MySQL.Async.execute('UPDATE `fsn_vehicles` SET `odometer` = odometer + @mileage WHERE `veh_plate` = @plate', { ['@plate'] = plate, ['@mileage'] = mileage }, function() end)
    end)
end)
...
AddEventHandler('fsn_odometer:setMileage', function(plate, mileage)
    MySQL.ready(function()
        MySQL.Async.execute('UPDATE `fsn_vehicles` SET `odometer` = @mileage WHERE `veh_plate` = @plate', { ['@plate'] = plate, ['@mileage'] = mileage }, function() end)
    end)
end)

RegisterServerEvent('fsn_odometer:resetMileage')
AddEventHandler('fsn_odometer:resetMileage', function(plate)
    TriggerEvent('fsn_odometer:setMileage', plate, 0)
end)
