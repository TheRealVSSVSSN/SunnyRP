	...
	if isPedDrivingAVehicle() then
		...
		if GetVehicleEngineHealth(vehicle) < cfg.cascadingFailureThreshold + 5 then
			if GetVehicleOilLevel(vehicle) > 0 then
				...
				TriggerEvent('fsn_notify:displayNotification', 'You repaired this vehicle.', 'centerLeft', 3000, 'success')
			else
				TriggerEvent('fsn_notify:displayNotification', 'You cannot repair this vehicle, contact a tow truck.', 'centerLeft', 3000, 'error')
			end
		else
			TriggerEvent('fsn_notify:displayNotification', 'This vehicle does not need repairing.<br>You <b>wasted</b> a repair kit!', 'centerLeft', 4000, 'error')
		end
	else
	...
	if isPedDrivingAVehicle() then
		...
		if GetVehicleEngineHealth(vehicle) < cfg.cascadingFailureThreshold + 5 then
			if GetVehicleOilLevel(vehicle) > 0 then
				...
				TriggerEvent('fsn_notify:displayNotification', 'You cannot repair this vehicle, contact a tow truck.', 'centerLeft', 4000, 'error')
			end
		else
			TriggerEvent('fsn_notify:displayNotification', 'This vehicle does not need repairing.', 'centerLeft', 4000, 'error')
		end
	else
		notification("~y~You must be in a vehicle to be able to repair it")
	end
