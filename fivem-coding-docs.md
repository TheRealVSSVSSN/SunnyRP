...
- **Purpose**: Returns whether the specified vehicle has its doors locked for a given player.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Vehicle handle.
  - `player` (`Player`): Player to test against.
  - **Returns**: `bool` locked state.
- **OneSync / Networking**: Call on the vehicle owner to guarantee accuracy in OneSync.
- **Examples**:
  - Lua:
    ```lua
    --[[
        -- Type: Command
        -- Name: doors_locked_ply
        -- Use: Prints if current vehicle doors are locked for self
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('doors_locked_ply', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
  ...
  - JavaScript:
    ```javascript
    /* Command: doors_locked_ply */
    RegisterCommand('doors_locked_ply', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      console.log(GetVehicleDoorsLockedForPlayer(veh, PlayerId()));
    });
    ```
- **Caveats / Limitations**:
  - Local lock state may desync if doors are forced open.
...
- **Purpose**: Retrieves the drivetrain type for a given vehicle model (FWD/RWD/AWD).
- **Parameters / Returns**:
  - `vehicleModel` (`Hash`): Model hash to query.
  - **Returns**: `int` drivetrain enum.
- **OneSync / Networking**: Model must be loaded on the client before querying.
- **Examples**:
  - Lua:
    ```lua
    --[[
        ...
        -- By: VSSVSSN
    --]]
    RegisterCommand('drive_train', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        local model = GetEntityModel(veh)
  ...
  - JavaScript:
    ...
    ```
- **Caveats / Limitations**:
  - Returns 0 if model not loaded or invalid.
...
- **Purpose**: Retrieves engine health, typically 0.0–1000.0.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Vehicle handle.
  - **Returns**: `float` health value.
- **OneSync / Networking**: Query owner to ensure up-to-date damage state.
- **Examples**:
  - Lua:
    ```lua
    --[[
  ...
  - JavaScript:
    ...
    ```
- **Caveats / Limitations**:
  - Values below 0 lead to engine failure.
...
- **Purpose**: Returns the environmental fade (paint fade) value of the vehicle.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Vehicle handle.
  - **Returns**: `float` fade value (0.0–1.0).
- **OneSync / Networking**: Changes should be applied on owner to replicate.
- **Examples**:
  - Lua:
    ```lua
    --[[
        -- Type: Command
        -- Name: paint_fade
        -- Use: Prints paint fade of current vehicle
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
...
- **Purpose**: Approximates the vehicle's maximum speed considering mods.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Vehicle handle.
  - **Returns**: `float` max speed in m/s.
- **OneSync / Networking**: Use on owner to reflect current modifications.
- **Examples**:
  - Lua:
    ```lua
    --[[
  ...
  - JavaScript:
    ...
    ```
- **Caveats / Limitations**:
  - Estimate may not match actual physics.
...
- **Purpose**: Retrieves pearlescent and wheel color indices.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Vehicle handle.
  - `pearlescentColor` (`int*`): Output pearlescent index.
  - `wheelColor` (`int*`): Output wheel color index.
- **OneSync / Networking**: Color changes replicate when set on owner.
- **Examples**:
  - Lua:
    ```lua
    --[[
        -- Type: Command
        -- Name: extra_colors
        -- Use: Prints pearlescent and wheel colors
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('extra_colors', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
  ...
  - JavaScript:
    ```javascript
    /* Command: extra_colors */
    RegisterCommand('extra_colors', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
    ...
    ```
- **Caveats / Limitations**:
  - Values map to predefined color tables.
...
- **Purpose**: Returns VTOL nozzle position ratio for VTOL aircraft.
- **Parameters / Returns**:
  - `aircraft` (`Vehicle`): Aircraft handle.
  - **Returns**: `float` nozzle ratio (0.0–1.0).
- **OneSync / Networking**: Use on owner; changes broadcast with corresponding setters.
- **Examples**:
  - Lua:
    ```lua
    --[[
        ...
        -- By: VSSVSSN
    --]]
    RegisterCommand('flight_nozzle', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
  ...
  - JavaScript:
    ```javascript
    /* Command: flight_nozzle */
    RegisterCommand('flight_nozzle', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      console.log(GetVehicleFlightNozzlePosition(veh));
    });
    ```
- **Caveats / Limitations**:
  - Applicable only to VTOL-capable vehicles.
...
- **Purpose**: Checks if the vehicle has KERS boost capability.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Vehicle handle.
  - **Returns**: `bool` indicating KERS presence.
- **OneSync / Networking**: KERS state is per vehicle; owner query recommended.
- **Examples**:
  - Lua:
    ```lua
    --[[
        -- Type: Command
        -- Name: has_kers
        -- Use: Prints if vehicle supports KERS
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
  ...
  - JavaScript:
    ...
    ```
- **Caveats / Limitations**:
  - Only specific vehicles support KERS.
...
- **Purpose**: Determines if the vehicle has a deployable parachute.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Vehicle handle.
  - **Returns**: `bool` parachute availability.
- **OneSync / Networking**: Query owner to check outfitted state.
- **Examples**:
  - Lua:
    ```lua
    --[[
  ...
  - JavaScript:
    ...
    ```
- **Caveats / Limitations**:
  - Applicable to vehicles with parachute mods.
...
- **Purpose**: Computes combined vehicle health as a percentage.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Vehicle handle.
  - **Returns**: `float` percentage 0.0–100.0.
- **OneSync / Networking**: Owner query recommended for accurate damage state.
- **Examples**:
  - Lua:
    ```lua
    --[[
  ...
  - JavaScript:
    ...
    ```
- **Caveats / Limitations**:
  - Derived from engine/body/petrol tank health; may not capture tyre damage.
...
- **Purpose**: Returns lock-on status for homing missiles targeting the vehicle.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Vehicle handle.
  - **Returns**: `int` state enum.
- **OneSync / Networking**: Call on owner to see latest targeting state.
- **Examples**:
  - Lua:
    ```lua
    --[[
...
- **Purpose**: Retrieves lock status for a specific door.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Vehicle handle.
  - `doorIndex` (`int`): Door enumeration index.
  - **Returns**: `int` door lock status.
- **OneSync / Networking**: Query owner to reflect real-time status.
- **Examples**:
  - Lua:
    ```lua
    --[[
        -- Type: Command
        -- Name: door_lock
        -- Use: Prints lock state of driver door
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
  ...
  - JavaScript:
    ...
    ```
- **Caveats / Limitations**:
  - Door indices follow `eDoorId` enumeration.
...
- **Purpose**: Obtains the interior color index.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Vehicle handle.
  - `color` (`int*`): Output color index.
- **OneSync / Networking**: Color queries should run on owner for reliability.
- **Examples**:
  - Lua:
    ```lua
    --[[
        -- Type: Command
        -- Name: interior_color
        -- Use: Prints interior color index
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('interior_color', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        print(_GET_VEHICLE_INTERIOR_COLOR(veh))
    end)
    ```
  - JavaScript:
    ```javascript
    ...
    RegisterCommand('interior_color', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      console.log(_GET_VEHICLE_INTERIOR_COLOR(veh));
    });
    ```
- **Caveats / Limitations**:
  - Returns color ID; map to palette separately.
...
- **Purpose**: Indicates if the vehicle belongs to mercenary hostile NPCs.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Vehicle handle.
  - **Returns**: `bool` mercenary flag.
- **OneSync / Networking**: Local check; ownership not required.
- **Examples**:
  - Lua:
    ```lua
    --[[
        -- Type: Command
        -- Name: merc_check
        -- Use: Prints if vehicle is a mercenary vehicle
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('merc_check', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
  ...
  - JavaScript:
    ```javascript
    /* Command: merc_check */
    RegisterCommand('merc_check', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      console.log(GetVehicleIsMercenary(veh));
    });
    ```
- **Caveats / Limitations**:
  - Typically used in NPC missions; rarely true for players.
...
- **Purpose**: Retrieves the seat layout configuration hash.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Vehicle handle.
  - **Returns**: `Hash` layout identifier.
- **OneSync / Networking**: Layout is static per model; ownership irrelevant.
- **Examples**:
  - Lua:
    ```lua
    --[[
        -- Type: Command
        -- Name: layout_hash
        -- Use: Prints vehicle layout hash
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
  ...
  - JavaScript:
    ...
    ```
- **Caveats / Limitations**:
  - Hash must be translated via lookup tables.
...
- **Purpose**: Checks if headlights and high-beams are active.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Vehicle handle.
  - `lightsOn` (`BOOL*`): Output if lights are on.
  - `highbeamsOn` (`BOOL*`): Output if high-beams are on.
  - **Returns**: `bool` success.
- **OneSync / Networking**: Ownership required to inspect accurate states.
- **Examples**:
  - Lua:
    ```lua
    --[[
        -- Type: Command
        -- Name: lights_state
        -- Use: Prints light and high-beam states
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('lights_state', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        local success, on, high = GetVehicleLightsState(veh)
        print(success, on, high)
    end)
    ```
  - JavaScript:
    ```javascript
    ...
    RegisterCommand('lights_state', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      const [success, on, high] = GetVehicleLightsState(veh);
      console.log(success, on, high);
    });
    ```
- **Caveats / Limitations**:
  - Success returns false if vehicle doesn't exist.
...
- **Purpose**: Returns the active livery index.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Vehicle handle.
  - **Returns**: `int` livery index.
- **OneSync / Networking**: Apply and query on owner to sync.
- **Examples**:
  - Lua:
    ```lua
    --[[
  ...
  - JavaScript:
    ...
    ```
- **Caveats / Limitations**:
  - Returns -1 if vehicle has no liveries.
...
- **Purpose**: Retrieves the total number of liveries available for the vehicle.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Vehicle handle.
  - **Returns**: `int` livery count.
- **OneSync / Networking**: Static per model; ownership not required.
- **Examples**:
  - Lua:
    ```lua
    --[[
  ...
  - JavaScript:
    ...
    ```
- **Caveats / Limitations**:
  - Returns 0 for models without liveries.
...
- **Purpose**: Retrieves the entity that the vehicle is lock-on targeting.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Vehicle handle.
  - `entity` (`Entity*`): Output targeted entity.
  - **Returns**: `bool` success.
- **OneSync / Networking**: Use on owner to read current targeting.
- **Examples**:
  - Lua:
    ```lua
    --[[
        -- Type: Command
        -- Name: lockon_target
        -- Use: Prints entity targeted by vehicle lock-on
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('lockon_target', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        local success, ent = GetVehicleLockOnTarget(veh)
        print(success, ent)
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: lockon_target */
    RegisterCommand('lockon_target', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      const [success, ent] = GetVehicleLockOnTarget(veh);
      console.log(success, ent);
    });
    ```
- **Caveats / Limitations**:
  - Returns false if no target.
...
- **Purpose**: Returns maximum braking capability considering mods.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Vehicle handle.
  - **Returns**: `float` braking value.
- **OneSync / Networking**: Query owner for modded vehicles.
- **Examples**:
  - Lua:
    ```lua
    --[[
  ...
  - JavaScript:
    ...
    ```
- **Caveats / Limitations**:
  - Metric is abstract; higher is better.
...
- **Purpose**: Returns the maximum number of passengers allowed.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Vehicle handle.
  - **Returns**: `int` passenger capacity.
- **OneSync / Networking**: Static per model; no ownership needed.
- **Examples**:
  - Lua:
    ```lua
    --[[
        -- Type: Command
        -- Name: max_pass
        -- Use: Prints passenger capacity
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('max_pass', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
  ...
  - JavaScript:
    ```javascript
    /* Command: max_pass */
    RegisterCommand('max_pass', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      console.log(GetVehicleMaxNumberOfPassengers(veh));
    });
    ```
- **Caveats / Limitations**:
  - Includes driver seat in count.
...
- **Purpose**: Returns maximum traction value for the vehicle.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Vehicle handle.
  - **Returns**: `float` traction stat.
- **OneSync / Networking**: Use on owner for modded vehicles.
- **Examples**:
  - Lua:
    ```lua
    --[[
        ...
        -- By: VSSVSSN
    --]]
    RegisterCommand('max_trac', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
  ...
  - JavaScript:
    ```javascript
    /* Command: max_trac */
    RegisterCommand('max_trac', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      console.log(GetVehicleMaxTraction(veh));
    });
    ```
- **Caveats / Limitations**:
  - Useful for tuning comparisons only.
...
- **Purpose**: Retrieves the installed modification index for a given mod slot.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Vehicle handle.
  - `modType` (`int`): Mod slot enumeration.
  - **Returns**: `int` installed mod index or -1.
- **OneSync / Networking**: Query owner to ensure sync with current mods.
- **Examples**:
  - Lua:
    ```lua
    --[[
        -- Type: Command
        -- Name: get_mod
        -- Use: Prints mod index for given slot
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('get_mod', function(source, args)
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        local slot = tonumber(args[1]) or 0
        print(GetVehicleMod(veh, slot))
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: get_mod */
    RegisterCommand('get_mod', (src, args) => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      const slot = parseInt(args[0]) || 0;
      console.log(GetVehicleMod(veh, slot));
    });
    ```
- **Caveats / Limitations**:
  - Slot IDs vary by vehicle; check mod kit first.
...
- **Purpose**: Fetches primary mod paint information.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Vehicle handle.
  - `paintType` (`int*`): Output paint type.
  - `color` (`int*`): Output color index.
  - `pearlescentColor` (`int*`): Output pearlescent index.
- **OneSync / Networking**: Mod color queries should be made on owner.
- **Examples**:
  - Lua:
    ```lua
    --[[
        -- Type: Command
        -- Name: mod_color1
        -- Use: Prints mod primary color data
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('mod_color1', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        local pType, color, pearl = GetVehicleModColor_1(veh)
        print(pType, color, pearl)
    end)
    ```
  - JavaScript:
    ```javascript
    ...
    RegisterCommand('mod_color1', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      const [pType, color, pearl] = GetVehicleModColor_1(veh);
      console.log(pType, color, pearl);
    });
    ```
- **Caveats / Limitations**:
  - Paint type corresponds to `eVehicleModType` color enums.
...
- **Purpose**: Returns a localized name for the primary mod color.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Vehicle handle.
...
- **Examples**:
  - Lua:
    ...
    --[[
        -- Type: Command
        -- Name: mod_color1_name
        -- Use: Prints mod primary color name
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('mod_color1_name', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        print(GetVehicleModColor_1Name(veh, true))
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: mod_color1_name */
    RegisterCommand('mod_color1_name', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      console.log(GetVehicleModColor_1Name(veh, true));
    });
    ```
- **Caveats / Limitations**:
...
- **Scope**: Shared
- **Signature(s)**: `BOOL IS_COP_VEHICLE_IN_AREA_3D(float x1, float x2, float y1, float y2, float z1, float z2)`
- **Purpose**: Checks if any law-enforcement vehicle exists within a 3D bounding box.
- **Parameters / Returns**:
  - `x1` (`float`), `y1` (`float`), `z1` (`float`): Minimum corner of the box.
  - `x2` (`float`), `y2` (`float`), `z2` (`float`): Maximum corner of the box.
  - **Returns**: `bool` indicating presence.
- **OneSync / Networking**: Pure spatial query; only considers vehicles streamed to the caller.
- **Examples**:
  - Lua:
    ```lua
    --[[
        -- Type: Command
        -- Name: cop_near
        -- Use: Checks for nearby cop vehicles
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('cop_near', function()
        local p = GetEntityCoords(PlayerPedId())
        local r = 50.0
        print(IsCopVehicleInArea_3d(p.x - r, p.x + r, p.y - r, p.y + r, p.z - r, p.z + r))
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: cop_near */
    RegisterCommand('cop_near', () => {
      const [x, y, z] = GetEntityCoords(PlayerPedId());
      const r = 50.0;
      console.log(IsCopVehicleInArea_3d(x - r, x + r, y - r, y + r, z - r, z + r));
    });
    ```
- **Caveats / Limitations**:
  - Does not detect unstreamed or server-only vehicles.
- **Reference**: https://docs.fivem.net/natives/?n=IsCopVehicleInArea_3d

##### IsEntityAttachedToHandlerFrame
- **Name**: IsEntityAttachedToHandlerFrame
- **Scope**: Shared
- **Signature(s)**: `BOOL IS_ENTITY_ATTACHED_TO_HANDLER_FRAME(Vehicle vehicle, Entity entity)`
- **Purpose**: Determines whether an entity is attached to a handler frame of a handler vehicle (e.g., cargo handlers).
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Handler vehicle.
  - `entity` (`Entity`): Target entity.
  - **Returns**: `bool` attachment state.
- **OneSync / Networking**: Run on the network owner of the handler vehicle for accurate results.
- **Examples**:
  - Lua:
    ```lua
    --[[
        -- Type: Command
        -- Name: handler_attached
        -- Use: Checks if entity is attached to handler frame
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('handler_attached', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        print(IsEntityAttachedToHandlerFrame(veh, PlayerPedId()))
    end)
    ```
  - JavaScript:
    ```javascript
    ...
    RegisterCommand('handler_attached', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      console.log(IsEntityAttachedToHandlerFrame(veh, PlayerPedId()));
    });
    ```
- **Caveats / Limitations**:
  - Only valid for vehicles with handler frames such as cargo loaders.
- **Reference**: https://docs.fivem.net/natives/?n=IsEntityAttachedToHandlerFrame

##### IsEntryPointForSeatClear
- **Name**: IsEntryPointForSeatClear
- **Scope**: Shared
- **Signature(s)**: `BOOL IS_ENTRY_POINT_FOR_SEAT_CLEAR(Ped ped, Vehicle vehicle, int seatIndex, BOOL side, BOOL onEnter)`
- **Purpose**: Tests whether a seat can be entered or exited without obstruction.
- **Parameters / Returns**:
  - `ped` (`Ped`): Evaluating ped.
  - `vehicle` (`Vehicle`): Target vehicle.
  - `seatIndex` (`int`): Seat to test (`-1` for driver).
  - `side` (`bool`): `false` for left, `true` for right on bikes.
  - `onEnter` (`bool`): `true` to check entering, `false` for exiting.
  - **Returns**: `bool` clear status.
- **OneSync / Networking**: Call on seat owner to avoid stale collision data.
- **Examples**:
  - Lua:
    ```lua
    --[[
        -- Type: Command
        -- Name: seat_clear
        -- Use: Reports if driver seat entry is clear
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('seat_clear', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), true)
        print(IsEntryPointForSeatClear(PlayerPedId(), veh, -1, false, true))
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: seat_clear */
    RegisterCommand('seat_clear', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), true);
      console.log(IsEntryPointForSeatClear(PlayerPedId(), veh, -1, false, true));
    });
    ```
- **Caveats / Limitations**:
  - Obstruction checks are local; physics desync can yield false positives.
- **Reference**: https://docs.fivem.net/natives/?n=IsEntryPointForSeatClear

##### IsExtraBrokenOff
- **Name**: IsExtraBrokenOff
- **Scope**: Shared
- **Signature(s)**: `BOOL IS_EXTRA_BROKEN_OFF(Vehicle vehicle, int extraId)`
- **Purpose**: Returns whether a specified vehicle extra is broken off or disabled.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Vehicle handle.
  - `extraId` (`int`): Extra component identifier.
  - **Returns**: `bool` broken state.
- **OneSync / Networking**: Use on vehicle owner to synchronize damage states.
- **Examples**:
  - Lua:
    ```lua
    --[[
        -- Type: Command
        -- Name: extra_broken
        -- Use: Checks if extra 1 is broken
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('extra_broken', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        print(IsExtraBrokenOff(veh, 1))
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: extra_broken */
    RegisterCommand('extra_broken', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      console.log(IsExtraBrokenOff(veh, 1));
    });
    ```
- **Caveats / Limitations**:
  - Non-breakable extras always return `false`.
- **Reference**: https://docs.fivem.net/natives/?n=IsExtraBrokenOff

##### IsHandlerFrameLinedUpWithContainer
- **Name**: IsHandlerFrameLinedUpWithContainer
- **Scope**: Shared
- **Signature(s)**: `BOOL IS_HANDLER_FRAME_LINED_UP_WITH_CONTAINER(Vehicle vehicle, Entity entity)`
- **Purpose**: Checks if a handler vehicle's frame is aligned with a container for pickup.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Handler vehicle.
  - `entity` (`Entity`): Target container entity.
  - **Returns**: `bool` alignment state.
- **OneSync / Networking**: Run on handler owner to ensure correct spatial testing.
- **Examples**:
  - Lua:
    ```lua
    --[[
        -- Type: Command
        -- Name: handler_align
        -- Use: Tests handler frame alignment
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('handler_align', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        print(IsHandlerFrameLinedUpWithContainer(veh, PlayerPedId()))
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: handler_align */
    RegisterCommand('handler_align', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      console.log(IsHandlerFrameLinedUpWithContainer(veh, PlayerPedId()));
    });
    ```
- **Caveats / Limitations**:
  - Applies only to handler vehicles capable of container operations.
- **Reference**: https://docs.fivem.net/natives/?n=IsHandlerFrameLinedUpWithContainer

##### IsHeliLandingAreaBlocked
- **Name**: IsHeliLandingAreaBlocked
- **Scope**: Shared
- **Signature(s)**: `BOOL IS_HELI_LANDING_AREA_BLOCKED(Vehicle vehicle)`
- **Purpose**: Detects if a helicopter's landing area is obstructed.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Helicopter handle.
  - **Returns**: `bool` blocked status.
- **OneSync / Networking**: Use on heli owner for accurate collision data.
- **Examples**:
  - Lua:
    ```lua
    --[[
        -- Type: Command
        -- Name: heli_blocked
        -- Use: Checks if landing zone is blocked
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('heli_blocked', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        print(IsHeliLandingAreaBlocked(veh))
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: heli_blocked */
    RegisterCommand('heli_blocked', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      console.log(IsHeliLandingAreaBlocked(veh));
    });
    ```
- **Caveats / Limitations**:
  - Only works on helicopters with landing skids.
- **Reference**: https://docs.fivem.net/natives/?n=IsHeliLandingAreaBlocked

##### IsHeliPartBroken
- **Name**: IsHeliPartBroken
- **Scope**: Shared
- **Signature(s)**: `BOOL IS_HELI_PART_BROKEN(Vehicle vehicle, BOOL p1, BOOL p2, BOOL p3)`
- **Purpose**: Returns whether specified helicopter components are damaged.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Helicopter handle.
  - `p1` (`bool`), `p2` (`bool`), `p3` (`bool`): Unknown component selectors.
  - **Returns**: `bool` damage state.
- **OneSync / Networking**: Query on vehicle owner to reflect local damage state.
- **Examples**:
  - Lua:
    ```lua
    --[[
        -- Type: Command
        -- Name: heli_part
        -- Use: Checks generic helicopter parts
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('heli_part', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        print(IsHeliPartBroken(veh, false, false, true))
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: heli_part */
    RegisterCommand('heli_part', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      console.log(IsHeliPartBroken(veh, false, false, true));
    });
    ```
- **Caveats / Limitations**:
  - Parameter meanings are undocumented.
  - TODO(next-run): verify component mapping.
- **Reference**: https://docs.fivem.net/natives/?n=IsHeliPartBroken

##### IsMissionTrain
- **Name**: IsMissionTrain
- **Scope**: Shared
- **Signature(s)**: `BOOL IS_MISSION_TRAIN(Vehicle train)`
- **Purpose**: Determines if a train is flagged as a mission train.
- **Parameters / Returns**:
  - `train` (`Vehicle`): Train vehicle handle.
  - **Returns**: `bool` mission status.
- **OneSync / Networking**: Call on train owner for authoritative status.
- **Examples**:
  - Lua:
    ```lua
    --[[
        -- Type: Command
        -- Name: mission_train
        -- Use: Reports if current train is mission-controlled
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('mission_train', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        print(IsMissionTrain(veh))
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: mission_train */
    RegisterCommand('mission_train', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      console.log(IsMissionTrain(veh));
    });
    ```
- **Caveats / Limitations**:
  - Only valid for train entities.
- **Reference**: https://docs.fivem.net/natives/?n=IsMissionTrain

##### IsNitrousActive
- **Name**: IsNitrousActive
- **Scope**: Shared
- **Signature(s)**: `BOOL IS_NITROUS_ACTIVE(Vehicle vehicle)`
- **Purpose**: Checks whether nitrous boost is active on a vehicle.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Vehicle handle.
  - **Returns**: `bool` nitrous status.
- **OneSync / Networking**: Query the owner to synchronize nitrous state across clients.
- **Examples**:
  - Lua:
    ```lua
    --[[
        -- Type: Command
        -- Name: nitrous_active
        -- Use: Prints nitrous status
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('nitrous_active', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        print(IsNitrousActive(veh))
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: nitrous_active */
    RegisterCommand('nitrous_active', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      console.log(IsNitrousActive(veh));
    });
    ```
- **Caveats / Limitations**:
  - Requires vehicles configured with nitrous mods.
- **Reference**: https://docs.fivem.net/natives/?n=IsNitrousActive

##### IsPedExclusiveDriverOfVehicle
- **Name**: IsPedExclusiveDriverOfVehicle
- **Scope**: Shared
- **Signature(s)**: `BOOL IS_PED_EXCLUSIVE_DRIVER_OF_VEHICLE(Ped ped, Vehicle vehicle, int* outIndex)`
- **Purpose**: Determines if a ped is the exclusive driver of a vehicle and optionally retrieves seat index.
- **Parameters / Returns**:
  - `ped` (`Ped`): Target ped.
  - `vehicle` (`Vehicle`): Vehicle in question.
  - `outIndex` (`int*`): Output seat index or `-1` if none.
  - **Returns**: `bool` exclusivity.
- **OneSync / Networking**: Use on vehicle owner to avoid mismatched seat data.
- **Examples**:
  - Lua:
    ```lua
    --[[
        -- Type: Command
        -- Name: exclusive_driver
        -- Use: Checks if player is sole driver
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('exclusive_driver', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        local idx = 0
        local result = IsPedExclusiveDriverOfVehicle(PlayerPedId(), veh, idx)
        print(result)
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: exclusive_driver */
    RegisterCommand('exclusive_driver', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      let out = 0;
      const result = IsPedExclusiveDriverOfVehicle(PlayerPedId(), veh, out);
      console.log(result);
    });
    ```
- **Caveats / Limitations**:
  - Output seat index is unreliable; treat as hint only.
- **Reference**: https://docs.fivem.net/natives/?n=IsPedExclusiveDriverOfVehicle

##### IsPlaneLandingGearIntact
- **Name**: IsPlaneLandingGearIntact
- **Scope**: Shared
- **Signature(s)**: `BOOL IS_PLANE_LANDING_GEAR_INTACT(Vehicle plane)`
- **Purpose**: Checks whether a plane's landing gear is intact.
- **Parameters / Returns**:
  - `plane` (`Vehicle`): Plane handle.
  - **Returns**: `bool` gear integrity.
- **OneSync / Networking**: Query on plane owner for accurate damage status.
- **Examples**:
  - Lua:
    ```lua
    --[[
        -- Type: Command
        -- Name: gear_intact
        -- Use: Prints landing gear status
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('gear_intact', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        print(IsPlaneLandingGearIntact(veh))
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: gear_intact */
    RegisterCommand('gear_intact', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      console.log(IsPlaneLandingGearIntact(veh));
    });
    ```
- **Caveats / Limitations**:
  - Only valid for fixed-wing aircraft.
- **Reference**: https://docs.fivem.net/natives/?n=IsPlaneLandingGearIntact

##### IsPlaybackGoingOnForVehicle
- **Name**: IsPlaybackGoingOnForVehicle
- **Scope**: Shared
- **Signature(s)**: `BOOL IS_PLAYBACK_GOING_ON_FOR_VEHICLE(Vehicle vehicle)`
- **Purpose**: Checks if a scripted vehicle recording playback is active.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Vehicle handle.
  - **Returns**: `bool` playback running.
- **OneSync / Networking**: Only authoritative on controlling node; other clients may not have playback info.
- **Examples**:
  - Lua:
    ```lua
    --[[
        ...
        -- Use: Prints if recording playback runs
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('playback_active', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        print(IsPlaybackGoingOnForVehicle(veh))
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: playback_active */
    RegisterCommand('playback_active', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      console.log(IsPlaybackGoingOnForVehicle(veh));
    });
    ```
- **Caveats / Limitations**:
  - Playback scripts must be started separately using recording natives.
- **Reference**: https://docs.fivem.net/natives/?n=IsPlaybackGoingOnForVehicle

##### IsPlaybackUsingAiGoingOnForVehicle
- **Name**: IsPlaybackUsingAiGoingOnForVehicle
- **Scope**: Shared
- **Signature(s)**: `BOOL IS_PLAYBACK_USING_AI_GOING_ON_FOR_VEHICLE(Vehicle vehicle)`
- **Purpose**: Indicates if a vehicle recording playback is using AI for navigation.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Vehicle handle.
  - **Returns**: `bool` AI playback status.
- **OneSync / Networking**: Only valid on the node orchestrating playback.
- **Examples**:
  - Lua:
    ```lua
    --[[
        -- Type: Command
        -- Name: playback_ai
        -- Use: Reports if AI playback runs
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('playback_ai', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        print(IsPlaybackUsingAiGoingOnForVehicle(veh))
    end)
    ```
  - JavaScript:
    ```javascript
    ...
    RegisterCommand('playback_ai', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      console.log(IsPlaybackUsingAiGoingOnForVehicle(veh));
    });
    ```
- **Caveats / Limitations**:
  - Requires playback started with AI flag.
- **Reference**: https://docs.fivem.net/natives/?n=IsPlaybackUsingAiGoingOnForVehicle

##### IsRocketBoostActive
- **Name**: IsRocketBoostActive
- **Scope**: Shared
- **Signature(s)**: `BOOL IS_ROCKET_BOOST_ACTIVE(Vehicle vehicle)`
- **Purpose**: Checks whether a vehicle's rocket boost is currently engaged.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Vehicle handle.
  - **Returns**: `bool` boost state.
- **OneSync / Networking**: Call on vehicle owner to sync boost state.
- **Examples**:
  - Lua:
    ```lua
    --[[
        -- Type: Command
        -- Name: rocket_boost
        -- Use: Prints rocket boost state
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('rocket_boost', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        print(IsRocketBoostActive(veh))
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: rocket_boost */
    RegisterCommand('rocket_boost', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      console.log(IsRocketBoostActive(veh));
    });
    ```
- **Caveats / Limitations**:
  - Only vehicles equipped with rocket boost return true.
- **Reference**: https://docs.fivem.net/natives/?n=IsRocketBoostActive

##### IsSeatWarpOnly
- **Name**: IsSeatWarpOnly
- **Scope**: Shared
- **Signature(s)**: `BOOL IS_SEAT_WARP_ONLY(Vehicle vehicle, int seatIndex)`
- **Purpose**: Determines if a vehicle seat can only be entered via warp.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Vehicle handle.
  - `seatIndex` (`int`): Target seat index.
  - **Returns**: `bool` warp-only flag.
- **OneSync / Networking**: Run on vehicle owner to respect seat restrictions.
- **Examples**:
  - Lua:
    ```lua
    --[[
        -- Type: Command
        -- Name: seat_warp
        -- Use: Checks if passenger seat requires warp
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('seat_warp', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        print(IsSeatWarpOnly(veh, 0))
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: seat_warp */
    RegisterCommand('seat_warp', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      console.log(IsSeatWarpOnly(veh, 0));
    });
    ```
- **Caveats / Limitations**:
  - Primarily used for scripted scenarios and special vehicles.
- **Reference**: https://docs.fivem.net/natives/?n=IsSeatWarpOnly

##### IsTaxiLightOn
- **Name**: IsTaxiLightOn
- **Scope**: Shared
- **Signature(s)**: `BOOL IS_TAXI_LIGHT_ON(Vehicle vehicle)`
- **Purpose**: Checks the state of a taxi's roof light.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Taxi vehicle.
  - **Returns**: `bool` light state.
- **OneSync / Networking**: Query on vehicle owner for up-to-date light status.
- **Examples**:
  - Lua:
    ```lua
    --[[
        -- Type: Command
        -- Name: taxi_light
        -- Use: Prints if taxi light is on
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('taxi_light', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        print(IsTaxiLightOn(veh))
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: taxi_light */
    RegisterCommand('taxi_light', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      console.log(IsTaxiLightOn(veh));
    });
    ```
- **Caveats / Limitations**:
  - Applies only to vehicles with taxi light extras.
- **Reference**: https://docs.fivem.net/natives/?n=IsTaxiLightOn

##### IsThisModelAnAmphibiousCar
- **Name**: IsThisModelAnAmphibiousCar
- **Scope**: Shared
- **Signature(s)**: `BOOL IS_THIS_MODEL_AN_AMPHIBIOUS_CAR(Hash model)`
- **Purpose**: Returns whether a model represents an amphibious car.
- **Parameters / Returns**:
  - `model` (`Hash`): Model identifier.
  - **Returns**: `bool` amphibious flag.
- **OneSync / Networking**: Pure model query; no network interaction.
- **Examples**:
  - Lua:
    ```lua
    --[[
        -- Type: Command
        -- Name: model_amph_car
        -- Use: Tests player's vehicle model for amphibious capability
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('model_amph_car', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
  ...
  - JavaScript:
    ```javascript
    /* Command: model_amph_car */
    RegisterCommand('model_amph_car', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      console.log(IsThisModelAnAmphibiousCar(GetEntityModel(veh)));
    });
    ```
- **Caveats / Limitations**:
  - Only certain water-capable cars return true.
- **Reference**: https://docs.fivem.net/natives/?n=IsThisModelAnAmphibiousCar

##### IsThisModelAnAmphibiousQuadbike
- **Name**: IsThisModelAnAmphibiousQuadbike
- **Scope**: Shared
- **Signature(s)**: `BOOL IS_THIS_MODEL_AN_AMPHIBIOUS_QUADBIKE(Hash model)`
- **Purpose**: Checks if a model is an amphibious quadbike.
- **Parameters / Returns**:
  - `model` (`Hash`): Model identifier.
  - **Returns**: `bool` amphibious flag.
- **OneSync / Networking**: Pure model query; no replication.
- **Examples**:
  - Lua:
    ```lua
    --[[
        -- Type: Command
        -- Name: model_amph_quad
        -- Use: Tests if vehicle is an amphibious quadbike
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('model_amph_quad', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
  ...
  - JavaScript:
    ```javascript
    /* Command: model_amph_quad */
    RegisterCommand('model_amph_quad', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      console.log(IsThisModelAnAmphibiousQuadbike(GetEntityModel(veh)));
    });
    ```
- **Caveats / Limitations**:
  - Returns false for standard quads.
- **Reference**: https://docs.fivem.net/natives/?n=IsThisModelAnAmphibiousQuadbike

##### IsThisModelABicycle
- **Name**: IsThisModelABicycle
- **Scope**: Shared
- **Signature(s)**: `BOOL IS_THIS_MODEL_A_BICYCLE(Hash model)`
- **Purpose**: Determines if the model represents a pedal bicycle.
- **Parameters / Returns**:
  - `model` (`Hash`): Model identifier.
  - **Returns**: `bool` bicycle flag.
- **OneSync / Networking**: Model lookup only; no network effect.
- **Examples**:
  - Lua:
    ```lua
    --[[
        -- Type: Command
        -- Name: model_bicycle
        -- Use: Checks if player's vehicle is a bicycle
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('model_bicycle', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        print(IsThisModelABicycle(GetEntityModel(veh)))
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: model_bicycle */
    RegisterCommand('model_bicycle', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      console.log(IsThisModelABicycle(GetEntityModel(veh)));
    });
    ```
- **Caveats / Limitations**:
  - Returns false for motorcycles.
- **Reference**: https://docs.fivem.net/natives/?n=IsThisModelABicycle

##### IsThisModelABike
- **Name**: IsThisModelABike
- **Scope**: Shared
- **Signature(s)**: `BOOL IS_THIS_MODEL_A_BIKE(Hash model)`
- **Purpose**: Checks if a model corresponds to a motorbike.
- **Parameters / Returns**:
  - `model` (`Hash`): Model identifier.
  - **Returns**: `bool` motorcycle flag.
- **OneSync / Networking**: Model lookup only.
- **Examples**:
  - Lua:
    ```lua
    --[[
        -- Type: Command
        -- Name: model_bike
        -- Use: Prints if vehicle model is a motorbike
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('model_bike', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        print(IsThisModelABike(GetEntityModel(veh)))
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: model_bike */
    RegisterCommand('model_bike', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      console.log(IsThisModelABike(GetEntityModel(veh)));
    });
    ```
- **Caveats / Limitations**:
  - Includes both motorcycles and quads.
- **Reference**: https://docs.fivem.net/natives/?n=IsThisModelABike

##### IsThisModelABoat
- **Name**: IsThisModelABoat
- **Scope**: Shared
- **Signature(s)**: `BOOL IS_THIS_MODEL_A_BOAT(Hash model)`
- **Purpose**: Determines if a model is categorized as a boat.
- **Parameters / Returns**:
  - `model` (`Hash`): Model identifier.
  - **Returns**: `bool` boat flag.
- **OneSync / Networking**: Pure model query.
- **Examples**:
  - Lua:
    ```lua
    --[[
        -- Type: Command
        -- Name: model_boat
        -- Use: Checks if vehicle model is a boat
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('model_boat', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        print(IsThisModelABoat(GetEntityModel(veh)))
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: model_boat */
    RegisterCommand('model_boat', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      console.log(IsThisModelABoat(GetEntityModel(veh)));
    });
    ```
- **Caveats / Limitations**:
  - Returns true for all watercraft including jetskis.
- **Reference**: https://docs.fivem.net/natives/?n=IsThisModelABoat

##### IsThisModelACar
- **Name**: IsThisModelACar
- **Scope**: Shared
- **Signature(s)**: `BOOL IS_THIS_MODEL_A_CAR(Hash model)`
- **Purpose**: Checks whether a model is classified as a standard automobile.
- **Parameters / Returns**:
  - `model` (`Hash`): Model identifier.
  - **Returns**: `bool` car flag.
- **OneSync / Networking**: Model lookup only.
- **Examples**:
  - Lua:
    ```lua
    --[[
        -- Type: Command
        -- Name: model_car
        -- Use: Prints if vehicle model is a car
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('model_car', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        print(IsThisModelACar(GetEntityModel(veh)))
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: model_car */
    RegisterCommand('model_car', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      console.log(IsThisModelACar(GetEntityModel(veh)));
    });
    ```
- **Caveats / Limitations**:
  - Use `IsThisModelAnAmphibiousCar` for water-capable cars.
- **Reference**: https://docs.fivem.net/natives/?n=IsThisModelACar

##### IsThisModelAHeli
- **Name**: IsThisModelAHeli
- **Scope**: Shared
- **Signature(s)**: `BOOL IS_THIS_MODEL_A_HELI(Hash model)`
- **Purpose**: Determines if a model is a helicopter.
- **Parameters / Returns**:
  - `model` (`Hash`): Model identifier.
  - **Returns**: `bool` helicopter flag.
- **OneSync / Networking**: Pure model query.
- **Examples**:
  - Lua:
    ```lua
    --[[
        -- Type: Command
        -- Name: model_heli
        -- Use: Checks if vehicle model is a helicopter
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('model_heli', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        print(IsThisModelAHeli(GetEntityModel(veh)))
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: model_heli */
    RegisterCommand('model_heli', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      console.log(IsThisModelAHeli(GetEntityModel(veh)));
    });
    ```
- **Caveats / Limitations**:
  - Includes VTOL aircraft flagged as helicopters.
- **Reference**: https://docs.fivem.net/natives/?n=IsThisModelAHeli

##### IsThisModelAJetski
- **Name**: IsThisModelAJetski
- **Scope**: Shared
- **Signature(s)**: `BOOL IS_THIS_MODEL_A_JETSKI(Hash model)`
- **Purpose**: Returns true if a model is a jetski.
- **Parameters / Returns**:
  - `model` (`Hash`): Model identifier.
  - **Returns**: `bool` jetski flag.
- **OneSync / Networking**: Pure model query.
- **Examples**:
  - Lua:
    ```lua
    --[[
        -- Type: Command
        -- Name: model_jetski
        -- Use: Prints if vehicle model is a jetski
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('model_jetski', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        print(IsThisModelAJetski(GetEntityModel(veh)))
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: model_jetski */
    RegisterCommand('model_jetski', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      console.log(IsThisModelAJetski(GetEntityModel(veh)));
    });
    ```
- **Caveats / Limitations**:
  - Detects jetskis even if classified as boats elsewhere.
- **Reference**: https://docs.fivem.net/natives/?n=IsThisModelAJetski

##### IsThisModelAPlane
- **Name**: IsThisModelAPlane
- **Scope**: Shared
- **Signature(s)**: `BOOL IS_THIS_MODEL_A_PLANE(Hash model)`
- **Purpose**: Determines if a model is a fixed-wing plane.
- **Parameters / Returns**:
  - `model` (`Hash`): Model identifier.
  - **Returns**: `bool` plane flag.
- **OneSync / Networking**: Pure model query with no replication.
- **Examples**:
  - Lua:
    ```lua
    --[[
        -- Type: Command
        -- Name: model_plane
        -- Use: Checks if vehicle model is a plane
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('model_plane', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        print(IsThisModelAPlane(GetEntityModel(veh)))
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: model_plane */
    RegisterCommand('model_plane', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      console.log(IsThisModelAPlane(GetEntityModel(veh)));
    });
    ```
- **Caveats / Limitations**:
...
- **Name**: IsVehicleModel
- **Scope**: Shared
- **Signature(s)**: `BOOL IS_VEHICLE_MODEL(Vehicle vehicle, Hash model)`
- **Purpose**: Returns true if a vehicle's model matches the given hash.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Vehicle handle.
  - `model` (`Hash`): Model hash to compare.
  - **Returns**: `bool` match flag.
- **OneSync / Networking**: Requires entity ownership for reliable model detection.
- **Examples**:
  - Lua:
    ```lua
    --[[
        -- Type: Command
        -- Name: veh_ismodel
        -- Use: Prints if player's vehicle is an Adder
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('veh_ismodel', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        print(IsVehicleModel(veh, `adder`))
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: veh_ismodel */
    RegisterCommand('veh_ismodel', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      console.log(IsVehicleModel(veh, GetHashKey('adder')));
    });
    ```
- **Caveats / Limitations**:
  - Model must be streamed in before comparison.
- **Reference**: https://docs.fivem.net/natives/?n=IsVehicleModel

##### IsVehicleNeonLightEnabled
- **Name**: IsVehicleNeonLightEnabled
- **Scope**: Shared
- **Signature(s)**: `BOOL _IS_VEHICLE_NEON_LIGHT_ENABLED(Vehicle vehicle, int index)`
- **Purpose**: Checks whether a specific neon light on a vehicle is enabled.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Vehicle handle.
  - `index` (`int`): Neon position (0–3: FL, FR, RL, RR).
  - **Returns**: `bool` enabled flag.
- **OneSync / Networking**: Neon state may not sync for vehicles you do not own.
- **Examples**:
  - Lua:
    ```lua
    --[[
        -- Type: Command
        -- Name: veh_neon
        -- Use: Prints if front-left neon is on
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('veh_neon', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        print(IsVehicleNeonLightEnabled(veh, 0))
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: veh_neon */
    RegisterCommand('veh_neon', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      console.log(IsVehicleNeonLightEnabled(veh, 0));
    });
    ```
- **Caveats / Limitations**:
  - Only valid for vehicles that support neon lights.
- **Reference**: https://docs.fivem.net/natives/?n=_IS_VEHICLE_NEON_LIGHT_ENABLED

##### IsVehicleOnAllWheels
- **Name**: IsVehicleOnAllWheels
- **Scope**: Shared
- **Signature(s)**: `BOOL IS_VEHICLE_ON_ALL_WHEELS(Vehicle vehicle)`
- **Purpose**: Returns true if the vehicle currently has all wheels touching the ground.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Vehicle to check.
  - **Returns**: `bool` all-wheels flag.
- **OneSync / Networking**: Physics queries require ownership for remote vehicles.
- **Examples**:
  - Lua:
    ```lua
    --[[
        -- Type: Command
        -- Name: veh_allwheels
        -- Use: Prints if vehicle is fully grounded
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('veh_allwheels', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        print(IsVehicleOnAllWheels(veh))
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: veh_allwheels */
    RegisterCommand('veh_allwheels', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      console.log(IsVehicleOnAllWheels(veh));
    });
    ```
- **Caveats / Limitations**:
  - Off-road physics may report false negatives on steep angles.
- **Reference**: https://docs.fivem.net/natives/?n=IsVehicleOnAllWheels

##### IsVehicleOnBoostPad
- **Name**: IsVehicleOnBoostPad
- **Scope**: Shared
- **Signature(s)**: `BOOL _IS_VEHICLE_ON_BOOST_PAD(Vehicle vehicle)`
- **Purpose**: Detects if the vehicle is currently on a stunt-race boost pad.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Vehicle handle.
  - **Returns**: `bool` boost-pad flag.
- **OneSync / Networking**: Local track data may not exist on the server.
- **Examples**:
  - Lua:
    ```lua
    --[[
        -- Type: Command
        -- Name: veh_boostpad
        -- Use: Prints if vehicle is on a boost pad
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('veh_boostpad', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        print(IsVehicleOnBoostPad(veh))
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: veh_boostpad */
    RegisterCommand('veh_boostpad', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      console.log(IsVehicleOnBoostPad(veh));
    });
    ```
- **Caveats / Limitations**:
  - Applicable mainly in stunt race contexts.
- **Reference**: https://docs.fivem.net/natives/?n=_IS_VEHICLE_ON_BOOST_PAD

##### IsVehicleParachuteActive
- **Name**: IsVehicleParachuteActive
- **Scope**: Shared
- **Signature(s)**: `BOOL _IS_VEHICLE_PARACHUTE_ACTIVE(Vehicle vehicle)`
- **Purpose**: Checks whether a vehicle's deployed parachute is active.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Target vehicle.
  - **Returns**: `bool` parachute state.
- **OneSync / Networking**: Parachute state syncs only for owned vehicles.
- **Examples**:
  - Lua:
    ```lua
    --[[
        -- Type: Command
        -- Name: veh_parachute
        -- Use: Prints if vehicle parachute is active
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('veh_parachute', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        print(IsVehicleParachuteActive(veh))
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: veh_parachute */
    RegisterCommand('veh_parachute', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      console.log(IsVehicleParachuteActive(veh));
    });
    ```
- **Caveats / Limitations**:
  - Only certain stunt vehicles support parachutes.
- **Reference**: https://docs.fivem.net/natives/?n=_IS_VEHICLE_PARACHUTE_ACTIVE

##### IsVehicleRocketBoostActive
- **Name**: IsVehicleRocketBoostActive
- **Scope**: Shared
- **Signature(s)**: `BOOL _IS_VEHICLE_ROCKET_BOOST_ACTIVE(Vehicle vehicle)`
- **Purpose**: Returns true while a vehicle's rocket boost is firing.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Vehicle with rocket boost.
  - **Returns**: `bool` boost state.
- **OneSync / Networking**: Requires ownership to detect active boost.
- **Examples**:
  - Lua:
    ```lua
    --[[
        -- Type: Command
        -- Name: veh_rocket
        -- Use: Prints if rocket boost is active
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('veh_rocket', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        print(IsVehicleRocketBoostActive(veh))
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: veh_rocket */
    RegisterCommand('veh_rocket', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      console.log(IsVehicleRocketBoostActive(veh));
    });
    ```
- **Caveats / Limitations**:
  - Only applies to vehicles equipped with rocket boost modules.
- **Reference**: https://docs.fivem.net/natives/?n=_IS_VEHICLE_ROCKET_BOOST_ACTIVE

##### IsVehicleSearchlightOn
- **Name**: IsVehicleSearchlightOn
- **Scope**: Shared
- **Signature(s)**: `BOOL IS_VEHICLE_SEARCHLIGHT_ON(Vehicle vehicle)`
- **Purpose**: Determines if a vehicle's searchlight is currently on.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Vehicle handle.
  - **Returns**: `bool` searchlight flag.
- **OneSync / Networking**: State replicates to others when owner toggles the light.
- **Examples**:
  - Lua:
    ```lua
    --[[
        -- Type: Command
        -- Name: veh_searchlight
        -- Use: Prints if searchlight is on
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('veh_searchlight', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        print(IsVehicleSearchlightOn(veh))
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: veh_searchlight */
    RegisterCommand('veh_searchlight', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      console.log(IsVehicleSearchlightOn(veh));
    });
    ```
- **Caveats / Limitations**:
  - Only applicable to vehicles equipped with searchlights.
- **Reference**: https://docs.fivem.net/natives/?n=IsVehicleSearchlightOn

##### IsVehicleSeatFree
- **Name**: IsVehicleSeatFree
- **Scope**: Shared
- **Signature(s)**: `BOOL IS_VEHICLE_SEAT_FREE(Vehicle vehicle, int seatIndex)`
- **Purpose**: Checks whether a specified seat in a vehicle is unoccupied.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Vehicle to inspect.
  - `seatIndex` (`int`): Seat identifier (e.g., `-1` driver).
  - **Returns**: `bool` seat availability.
- **OneSync / Networking**: Seat occupancy syncs for networked vehicles.
- **Examples**:
  - Lua:
    ```lua
    --[[
        -- Type: Command
        -- Name: seat_free
        -- Use: Prints if passenger seat is free
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('seat_free', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        print(IsVehicleSeatFree(veh, 0))
    end)
    ```
  - JavaScript:
    ```javascript
    ...
    RegisterCommand('seat_free', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      console.log(IsVehicleSeatFree(veh, 0));
    });
    ```
- **Caveats / Limitations**:
  - Seat indices vary across vehicle types.
- **Reference**: https://docs.fivem.net/natives/?n=IsVehicleSeatFree

##### IsVehicleSirenAudioOn
- **Name**: IsVehicleSirenAudioOn
- **Scope**: Shared
- **Signature(s)**: `BOOL IS_VEHICLE_SIREN_AUDIO_ON(Vehicle vehicle)`
- **Purpose**: Checks if the vehicle's siren sound is active.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Vehicle to check.
  - **Returns**: `bool` audio active flag.
- **OneSync / Networking**: Siren audio state is replicated from the controlling client.
- **Examples**:
  - Lua:
    ```lua
    --[[
        -- Type: Command
        -- Name: siren_audio
        -- Use: Prints if siren audio is on
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('siren_audio', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        print(IsVehicleSirenAudioOn(veh))
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: siren_audio */
    RegisterCommand('siren_audio', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      console.log(IsVehicleSirenAudioOn(veh));
    });
    ```
- **Caveats / Limitations**:
  - Audio may be muted if `MuteSirens` is enabled.
- **Reference**: https://docs.fivem.net/natives/?n=IsVehicleSirenAudioOn

##### IsVehicleSirenOn
- **Name**: IsVehicleSirenOn
- **Scope**: Shared
- **Signature(s)**: `BOOL IS_VEHICLE_SIREN_ON(Vehicle vehicle)`
- **Purpose**: Returns true if vehicle emergency lights are active.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Vehicle to check.
  - **Returns**: `bool` siren state.
- **OneSync / Networking**: Requires entity ownership for state changes.
- **Examples**:
  - Lua:
    ```lua
    --[[
        -- Type: Command
        -- Name: siren_on
        -- Use: Prints if siren lights are on
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('siren_on', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        print(IsVehicleSirenOn(veh))
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: siren_on */
    RegisterCommand('siren_on', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      console.log(IsVehicleSirenOn(veh));
    });
    ```
- **Caveats / Limitations**:
  - Lights can be active without audio.
- **Reference**: https://docs.fivem.net/natives/?n=IsVehicleSirenOn

##### IsVehicleSlipstreamLeader
- **Name**: IsVehicleSlipstreamLeader
- **Scope**: Shared
- **Signature(s)**: `BOOL _IS_VEHICLE_SLIPSTREAM_LEADER(Vehicle vehicle)`
- **Purpose**: Determines if a vehicle is the lead vehicle in a slipstream chain.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Vehicle to check.
  - **Returns**: `bool` leader flag.
- **OneSync / Networking**: Slipstream data is local to race scripts.
- **Examples**:
  - Lua:
    ```lua
    --[[
        -- Type: Command
        -- Name: slip_leader
        -- Use: Prints if vehicle is slipstream leader
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('slip_leader', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        print(IsVehicleSlipstreamLeader(veh))
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: slip_leader */
    RegisterCommand('slip_leader', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      console.log(IsVehicleSlipstreamLeader(veh));
    });
    ```
- **Caveats / Limitations**:
  - Only relevant in slipstream-enabled races.
- **Reference**: https://docs.fivem.net/natives/?n=_IS_VEHICLE_SLIPSTREAM_LEADER

##### IsVehicleSprayable
- **Name**: IsVehicleSprayable
- **Scope**: Shared
- **Signature(s)**: `BOOL IS_VEHICLE_SPRAYABLE(Vehicle vehicle)`
- **Purpose**: Returns whether the vehicle can be resprayed in mod shops.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Vehicle handle.
  - **Returns**: `bool` sprayable flag.
- **OneSync / Networking**: Pure lookup; no replication concerns.
- **Examples**:
  - Lua:
    ```lua
    --[[
        -- Type: Command
        -- Name: veh_sprayable
        -- Use: Prints if vehicle can be resprayed
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('veh_sprayable', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        print(IsVehicleSprayable(veh))
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: veh_sprayable */
    RegisterCommand('veh_sprayable', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      console.log(IsVehicleSprayable(veh));
    });
    ```
- **Caveats / Limitations**:
  - Certain mission vehicles cannot be resprayed.
- **Reference**: https://docs.fivem.net/natives/?n=IsVehicleSprayable

##### IsVehicleStolen
- **Name**: IsVehicleStolen
- **Scope**: Shared
- **Signature(s)**: `BOOL IS_VEHICLE_STOLEN(Vehicle vehicle)`
- **Purpose**: Indicates whether the vehicle is flagged as stolen.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Vehicle handle.
  - **Returns**: `bool` stolen flag.
- **OneSync / Networking**: Stolen state replicates with entity state.
- **Examples**:
  - Lua:
    ```lua
    --[[
        -- Type: Command
        -- Name: veh_stolen
        -- Use: Prints if vehicle is marked stolen
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('veh_stolen', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        print(IsVehicleStolen(veh))
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: veh_stolen */
    RegisterCommand('veh_stolen', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      console.log(IsVehicleStolen(veh));
    });
    ```
- **Caveats / Limitations**:
  - Many vehicles default to not stolen even if player is not owner.
- **Reference**: https://docs.fivem.net/natives/?n=IsVehicleStolen

##### IsVehicleStopped
- **Name**: IsVehicleStopped
- **Scope**: Shared
- **Signature(s)**: `BOOL IS_VEHICLE_STOPPED(Vehicle vehicle)`
- **Purpose**: Returns true if vehicle speed is near zero.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Vehicle handle.
  - **Returns**: `bool` stopped flag.
- **OneSync / Networking**: Requires ownership for accurate speed reading.
- **Examples**:
  - Lua:
    ```lua
    --[[
        -- Type: Command
        -- Name: veh_stopped
        -- Use: Prints if vehicle is stopped
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('veh_stopped', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        print(IsVehicleStopped(veh))
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: veh_stopped */
    RegisterCommand('veh_stopped', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      console.log(IsVehicleStopped(veh));
    });
    ```
- **Caveats / Limitations**:
  - Small movements may still report not stopped.
- **Reference**: https://docs.fivem.net/natives/?n=IsVehicleStopped

##### IsVehicleStoppedAtTrafficLights
- **Name**: IsVehicleStoppedAtTrafficLights
- **Scope**: Shared
- **Signature(s)**: `BOOL IS_VEHICLE_STOPPED_AT_TRAFFIC_LIGHTS(Vehicle vehicle)`
- **Purpose**: Checks if the vehicle is stopped at a traffic light.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Vehicle handle.
  - **Returns**: `bool` stoplight flag.
- **OneSync / Networking**: Depends on client-side traffic light detection.
- **Examples**:
  - Lua:
    ```lua
    --[[
        -- Type: Command
        -- Name: veh_stoplight
        -- Use: Prints if vehicle is at a red light
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('veh_stoplight', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        print(IsVehicleStoppedAtTrafficLights(veh))
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: veh_stoplight */
    RegisterCommand('veh_stoplight', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      console.log(IsVehicleStoppedAtTrafficLights(veh));
    });
    ```
- **Caveats / Limitations**:
  - Only works for mapped traffic light positions.
- **Reference**: https://docs.fivem.net/natives/?n=IsVehicleStoppedAtTrafficLights

##### IsVehicleStuckOnRoof
- **Name**: IsVehicleStuckOnRoof
- **Scope**: Shared
- **Signature(s)**: `BOOL IS_VEHICLE_STUCK_ON_ROOF(Vehicle vehicle)`
- **Purpose**: Detects if a vehicle is resting upside-down.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Vehicle handle.
  - **Returns**: `bool` stuck flag.
- **OneSync / Networking**: Physics state relies on entity ownership.
- **Examples**:
  - Lua:
    ```lua
    --[[
        -- Type: Command
        -- Name: veh_roof
        -- Use: Prints if vehicle is stuck on its roof
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('veh_roof', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        print(IsVehicleStuckOnRoof(veh))
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: veh_roof */
    RegisterCommand('veh_roof', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      console.log(IsVehicleStuckOnRoof(veh));
    });
    ```
- **Caveats / Limitations**:
  - May not detect short flips.
- **Reference**: https://docs.fivem.net/natives/?n=IsVehicleStuckOnRoof

##### IsVehicleStuckTimerUp
- **Name**: IsVehicleStuckTimerUp
- **Scope**: Shared
- **Signature(s)**: `BOOL IS_VEHICLE_STUCK_TIMER_UP(Vehicle vehicle, int p1, int p2)`
- **Purpose**: Tests if a vehicle has been stuck for specified durations.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Vehicle handle.
  - `p1` (`int`): Timer index.
  - `p2` (`int`): Time in ms.
  - **Returns**: `bool` timer expired flag.
- **OneSync / Networking**: Timer state is local to the controlling script.
- **Examples**:
  - Lua:
    ```lua
    --[[
        -- Type: Command
        -- Name: veh_stucktimer
        -- Use: Prints if vehicle has been stuck for 5s
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('veh_stucktimer', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        print(IsVehicleStuckTimerUp(veh, 0, 5000))
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: veh_stucktimer */
    RegisterCommand('veh_stucktimer', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      console.log(IsVehicleStuckTimerUp(veh, 0, 5000));
    });
    ```
- **Caveats / Limitations**:
  - Timer indices 0–3 represent different stuck conditions.
- **Reference**: https://docs.fivem.net/natives/?n=IsVehicleStuckTimerUp

##### IsVehicleTyreBurst
- **Name**: IsVehicleTyreBurst
- **Scope**: Shared
- **Signature(s)**: `BOOL IS_VEHICLE_TYRE_BURST(Vehicle vehicle, int wheelID, BOOL isBurstToRim)`
- **Purpose**: Checks if a specific tyre is burst.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Vehicle handle.
  - `wheelID` (`int`): Wheel index.
  - `isBurstToRim` (`bool`): Treats fully shredded tyres as burst.
  - **Returns**: `bool` burst flag.
- **OneSync / Networking**: Tyre damage requires entity ownership.
- **Examples**:
  - Lua:
    ```lua
    --[[
        -- Type: Command
        -- Name: tyre_check
        -- Use: Prints if front-left tyre is burst
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('tyre_check', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        print(IsVehicleTyreBurst(veh, 0, true))
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: tyre_check */
    RegisterCommand('tyre_check', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      console.log(IsVehicleTyreBurst(veh, 0, true));
    });
    ```
- **Caveats / Limitations**:
  - Wheel indices differ on motorcycles and larger vehicles.
- **Reference**: https://docs.fivem.net/natives/?n=IsVehicleTyreBurst

##### IsVehicleVisible
- **Name**: IsVehicleVisible
- **Scope**: Shared
- **Signature(s)**: `BOOL IS_VEHICLE_VISIBLE(Vehicle vehicle)`
- **Purpose**: Checks whether the vehicle is currently visible (not hidden or occluded).
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Vehicle handle.
  - **Returns**: `bool` visibility flag.
- **OneSync / Networking**: Hidden state may differ per client depending on streaming.
- **Examples**:
  - Lua:
    ```lua
    --[[
        -- Type: Command
        -- Name: veh_visible
        -- Use: Prints if vehicle is visible
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('veh_visible', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        print(IsVehicleVisible(veh))
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: veh_visible */
    RegisterCommand('veh_visible', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      console.log(IsVehicleVisible(veh));
    });
    ```
- **Caveats / Limitations**:
  - `SetEntityVisible` may override normal checks.
- **Reference**: https://docs.fivem.net/natives/?n=IsVehicleVisible

##### IsVehicleWeaponDisabled
- **Name**: IsVehicleWeaponDisabled
- **Scope**: Shared
- **Signature(s)**: `BOOL _IS_VEHICLE_WEAPON_DISABLED(Hash weaponHash, Vehicle vehicle, Ped owner)`
- **Purpose**: Determines if a specific vehicle-mounted weapon is disabled for a driver.
- **Parameters / Returns**:
  - `weaponHash` (`Hash`): Weapon identifier.
  - `vehicle` (`Vehicle`): Vehicle handle.
  - `owner` (`Ped`): Intended owner of the weapon.
  - **Returns**: `bool` disabled flag.
- **OneSync / Networking**: Weapon disable state must be set by owning server/client.
- **Examples**:
  - Lua:
    ```lua
    --[[
        -- Type: Command
        -- Name: veh_weapon_disabled
        -- Use: Prints if vehicle MG is disabled
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('veh_weapon_disabled', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        print(IsVehicleWeaponDisabled(`VEHICLE_WEAPON_TANK`, veh, PlayerPedId()))
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: veh_weapon_disabled */
    RegisterCommand('veh_weapon_disabled', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      console.log(IsVehicleWeaponDisabled(GetHashKey('VEHICLE_WEAPON_TANK'), veh, PlayerPedId()));
    });
    ```
- **Caveats / Limitations**:
  - Requires valid weapon hashes for the given vehicle.
- **Reference**: https://docs.fivem.net/natives/?n=_IS_VEHICLE_WEAPON_DISABLED

##### IsVehicleWindowIntact
- **Name**: IsVehicleWindowIntact
- **Scope**: Shared
- **Signature(s)**: `BOOL IS_VEHICLE_WINDOW_INTACT(Vehicle vehicle, int windowIndex)`
- **Purpose**: Checks if a specified window on a vehicle is unbroken.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Vehicle handle.
  - `windowIndex` (`int`): Window identifier.
  - **Returns**: `bool` intact flag.
- **OneSync / Networking**: Window damage synchronizes with entity owner.
- **Examples**:
  - Lua:
    ```lua
    --[[
        -- Type: Command
        -- Name: window_intact
        -- Use: Prints if driver's window is intact
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('window_intact', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        print(IsVehicleWindowIntact(veh, 0))
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: window_intact */
    RegisterCommand('window_intact', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      console.log(IsVehicleWindowIntact(veh, 0));
    });
    ```
- **Caveats / Limitations**:
  - Indices vary with vehicle door counts.
- **Reference**: https://docs.fivem.net/natives/?n=IsVehicleWindowIntact

##### LowerConvertibleRoof
- **Name**: LowerConvertibleRoof
- **Scope**: Shared
- **Signature(s)**: `void LOWER_CONVERTIBLE_ROOF(Vehicle vehicle, BOOL instantlyLower)`
- **Purpose**: Lowers a convertible's roof, optionally instant.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Convertible vehicle.
  - `instantlyLower` (`bool`): If true, skips animation.
- **OneSync / Networking**: Roof state changes replicate to others.
- **Examples**:
  - Lua:
    ```lua
    --[[
        -- Type: Command
        -- Name: roof_down
        -- Use: Lowers convertible roof instantly
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('roof_down', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
  ...
  - JavaScript:
    ```javascript
    /* Command: roof_down */
    RegisterCommand('roof_down', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      LowerConvertibleRoof(veh, true);
    });
    ```
- **Caveats / Limitations**:
  - Only works on vehicles flagged as convertibles.
- **Reference**: https://docs.fivem.net/natives/?n=LowerConvertibleRoof

##### LowerRetractableWheels
- **Name**: LowerRetractableWheels
- **Scope**: Shared
- **Signature(s)**: `void _LOWER_RETRACTABLE_WHEELS(Vehicle vehicle)`
- **Purpose**: Extends retractable landing gear or wheels on applicable vehicles.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Vehicle with retractable wheels.
- **OneSync / Networking**: Gear state requires entity ownership to replicate.
- **Examples**:
  - Lua:
    ```lua
    --[[
        -- Type: Command
        -- Name: wheels_down
        -- Use: Lowers retractable wheels
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('wheels_down', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        LowerRetractableWheels(veh)
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: wheels_down */
    RegisterCommand('wheels_down', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      LowerRetractableWheels(veh);
    });
    ```
- **Caveats / Limitations**:
  - Not all aircraft support retractable gear.
- **Reference**: https://docs.fivem.net/natives/?n=_LOWER_RETRACTABLE_WHEELS

##### ModifyVehicleTopSpeed
- **Name**: ModifyVehicleTopSpeed
- **Scope**: Shared
- **Signature(s)**: `void MODIFY_VEHICLE_TOP_SPEED(Vehicle vehicle, float value)`
- **Purpose**: Adjusts a vehicle's max speed by a multiplier value.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Vehicle handle.
  - `value` (`float`): Speed multiplier (-1.0 resets).
- **OneSync / Networking**: Applied on owner; state syncs via physics.
- **Examples**:
  - Lua:
    ```lua
    --[[
        -- Type: Command
        -- Name: speed_mod
        -- Use: Sets vehicle top speed multiplier
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('speed_mod', function(source, args)
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
  ...
  - JavaScript:
    ```javascript
    /* Command: speed_mod */
    RegisterCommand('speed_mod', (src, args) => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
    ...
    ```
- **Caveats / Limitations**:
  - Negative multiplier resets to handling-defined speed.
- **Reference**: https://docs.fivem.net/natives/?n=ModifyVehicleTopSpeed

##### _0x0205F5365292D2EB
- **Name**: _0x0205F5365292D2EB
- **Scope**: Shared
- **Signature(s)**: `void _0x0205F5365292D2EB(Vehicle vehicle, float p1)`
- **Purpose**: Undocumented native affecting vehicle dynamics.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Vehicle handle.
  - `p1` (`float`): Unknown effect.
- **OneSync / Networking**: Requires entity ownership.
- **Examples**:
  - Lua:
    ```lua
    --[[
        -- Type: Command
        -- Name: veh_0205
        -- Use: Calls undocumented native on vehicle
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('veh_0205', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        _0x0205F5365292D2EB(veh, 1.0)
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: veh_0205 */
    RegisterCommand('veh_0205', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      _0x0205F5365292D2EB(veh, 1.0);
    });
    ```
...
##### _0x8664170EF165C4A6
- **Name**: _0x8664170EF165C4A6
- **Scope**: Client
- **Signature**: `void _0x8664170EF165C4A6(Any p0, Any p1);`
...
- **Examples**:
  - Lua:
    ...
    --[[
        -- Type: Command
        -- Name: veh_8664
        -- Use: Demonstrates _0x8664170EF165C4A6
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('veh_8664', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        _0x8664170EF165C4A6(veh, 0, 0)
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: veh_8664 */
    RegisterCommand('veh_8664', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      _0x8664170EF165C4A6(veh, 0, 0);
    });
    ```
- **Caveats / Limitations**:
  - Undocumented native.
  - TODO(next-run): verify semantics.
- **Reference**: https://docs.fivem.net/natives/?_0x8664170EF165C4A6

##### _0x870B8B7A766615C8
- **Name**: _0x870B8B7A766615C8
- **Scope**: Client
- **Signature**: `void _0x870B8B7A766615C8(Any p0, Any p1, Any p2);`
- **Purpose**: Undocumented native with three unknown parameters.
- **Parameters / Returns**:
  - `p0` (`Any`): Undocumented.
  - `p1` (`Any`): Undocumented.
  - `p2` (`Any`): Undocumented.
...
- **Examples**:
  - Lua:
    ...
    --[[
        -- Type: Command
        -- Name: veh_870b
        -- Use: Demonstrates _0x870B8B7A766615C8
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('veh_870b', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        _0x870B8B7A766615C8(veh, 0, 0)
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: veh_870b */
    RegisterCommand('veh_870b', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      _0x870B8B7A766615C8(veh, 0, 0);
    });
    ```
- **Caveats / Limitations**:
  - Undocumented native.
  - TODO(next-run): verify semantics.
- **Reference**: https://docs.fivem.net/natives/?_0x870B8B7A766615C8

##### _0x8821196D91FA2DE5
- **Name**: _0x8821196D91FA2DE5
- **Scope**: Client
- **Signature**: `void _0x8821196D91FA2DE5(Vehicle vehicle, BOOL toggle);`
- **Purpose**: Undocumented native toggling an unknown vehicle state.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Target vehicle.
  - `toggle` (`BOOL`): Enable or disable.
  - **Returns**: `void` None.
...
- **Examples**:
  - Lua:
    ...
    --[[
        -- Type: Command
        -- Name: veh_8821
        -- Use: Demonstrates _0x8821196D91FA2DE5
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('veh_8821', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        _0x8821196D91FA2DE5(veh, true)
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: veh_8821 */
    RegisterCommand('veh_8821', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      _0x8821196D91FA2DE5(veh, true);
    });
    ```
- **Caveats / Limitations**:
  - Behavior unclear in official docs.
  - TODO(next-run): verify semantics.
- **Reference**: https://docs.fivem.net/natives/?_0x8821196D91FA2DE5

##### _0x887FA38787DE8C72
- **Name**: _0x887FA38787DE8C72
- **Scope**: Client
- **Signature**: `void _0x887FA38787DE8C72(Vehicle vehicle);`
- **Purpose**: Undocumented native performing an unknown vehicle action.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Target vehicle.
  - **Returns**: `void` None.
- **OneSync / Networking**: Run on vehicle owner for replication.
- **Examples**:
  - Lua:
    ...
    --[[
        -- Type: Command
        -- Name: veh_887f
        -- Use: Demonstrates _0x887FA38787DE8C72
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('veh_887f', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        _0x887FA38787DE8C72(veh)
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: veh_887f */
    RegisterCommand('veh_887f', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      _0x887FA38787DE8C72(veh);
    });
    ```
- **Caveats / Limitations**:
  - Undocumented native.
- **Reference**: https://docs.fivem.net/natives/?_0x887FA38787DE8C72

##### _0x88BC673CA9E0AE99
- **Name**: _0x88BC673CA9E0AE99
- **Scope**: Client
- **Signature**: `void _0x88BC673CA9E0AE99(Vehicle vehicle, BOOL p1);`
- **Purpose**: Undocumented native; toggles an unknown vehicle feature.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Target vehicle.
  - `p1` (`BOOL`): Unknown toggle.
  - **Returns**: `void` None.
- **OneSync / Networking**: Run on vehicle owner for replication.
- **Examples**:
  - Lua:
    ...
    --[[
        -- Type: Command
        -- Name: veh_88bc
        -- Use: Demonstrates _0x88BC673CA9E0AE99
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('veh_88bc', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        _0x88BC673CA9E0AE99(veh, true)
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: veh_88bc */
    RegisterCommand('veh_88bc', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      _0x88BC673CA9E0AE99(veh, true);
    });
    ```
- **Caveats / Limitations**:
  - Undocumented native.
  - TODO(next-run): verify behavior.
- **Reference**: https://docs.fivem.net/natives/?_0x88BC673CA9E0AE99

##### _ENABLE_AIRCRAFT_OBSTACLE_AVOIDANCE (_0x8AA9180DE2FEDD45)
- **Name**: _ENABLE_AIRCRAFT_OBSTACLE_AVOIDANCE
- **Scope**: Client
- **Signature**: `void _ENABLE_AIRCRAFT_OBSTACLE_AVOIDANCE(Vehicle vehicle, BOOL avoidObstacles);`
- **Purpose**: Toggles AI obstacle avoidance for aircraft, affecting how pilots avoid terrain.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Target aircraft.
  - `avoidObstacles` (`BOOL`): `true` to enable avoidance, `false` to disable.
  - **Returns**: `void` None.
- **OneSync / Networking**: Run on vehicle owner; AI behavior must execute on controlling client/server.
- **Examples**:
  - Lua:
    ```lua
    --[[
        -- Type: Command
        -- Name: veh_8aa9
        -- Use: Demonstrates _ENABLE_AIRCRAFT_OBSTACLE_AVOIDANCE
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('veh_8aa9', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        _ENABLE_AIRCRAFT_OBSTACLE_AVOIDANCE(veh, false)
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: veh_8aa9 */
    RegisterCommand('veh_8aa9', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      _ENABLE_AIRCRAFT_OBSTACLE_AVOIDANCE(veh, false);
    });
    ```
- **Caveats / Limitations**:
  - Disabling avoidance may cause AI crashes.
- **Reference**: https://docs.fivem.net/natives/?_0x8AA9180DE2FEDD45

##### _0x8F0D5BA1C2CC91D7
- **Name**: _0x8F0D5BA1C2CC91D7
- **Scope**: Client
- **Signature**: `void _0x8F0D5BA1C2CC91D7(BOOL toggle);`
- **Purpose**: Undocumented native toggling an unknown global vehicle flag.
- **Parameters / Returns**:
  - `toggle` (`BOOL`): Unknown toggle.
  - **Returns**: `void` None.
- **OneSync / Networking**: Global flag; ensure consistent state across clients.
- **Examples**:
  - Lua:
    ```lua
    --[[
        -- Type: Command
        -- Name: veh_8f0d
        -- Use: Demonstrates _0x8F0D5BA1C2CC91D7
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('veh_8f0d', function()
        _0x8F0D5BA1C2CC91D7(true)
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: veh_8f0d */
    RegisterCommand('veh_8f0d', () => {
      _0x8F0D5BA1C2CC91D7(true);
    });
    ```
- **Caveats / Limitations**:
  - Undocumented native.
  - TODO(next-run): verify behavior.
- **Reference**: https://docs.fivem.net/natives/?_0x8F0D5BA1C2CC91D7

##### _0x9640E30A7F395E4B
- **Name**: _0x9640E30A7F395E4B
- **Scope**: Client
- **Signature**: `void _0x9640E30A7F395E4B(Vehicle vehicle, Any p1, Any p2, Any p3, Any p4);`
- **Purpose**: Undocumented native with multiple unknown parameters.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Target vehicle.
  - `p1` (`Any`): Unknown.
  - `p2` (`Any`): Unknown.
...
- **Examples**:
  - Lua:
    ...
    --[[
        -- Type: Command
        -- Name: veh_9640
        -- Use: Demonstrates _0x9640E30A7F395E4B
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('veh_9640', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        _0x9640E30A7F395E4B(veh, 0, 0, 0, 0)
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: veh_9640 */
    RegisterCommand('veh_9640', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      _0x9640E30A7F395E4B(veh, 0, 0, 0, 0);
    });
    ```
- **Caveats / Limitations**:
  - Undocumented native.
  - TODO(next-run): verify semantics.
- **Reference**: https://docs.fivem.net/natives/?_0x9640E30A7F395E4B

##### _0x97841634EF7DF1D6
- **Name**: _0x97841634EF7DF1D6
- **Scope**: Client
- **Signature**: `void _0x97841634EF7DF1D6(Vehicle vehicle, BOOL toggle);`
- **Purpose**: Undocumented native toggling an unknown vehicle state.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Target vehicle.
  - `toggle` (`BOOL`): Enable or disable.
...
- **Examples**:
  - Lua:
    ...
    --[[
        -- Type: Command
        -- Name: veh_9784
        -- Use: Demonstrates _0x97841634EF7DF1D6
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('veh_9784', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        _0x97841634EF7DF1D6(veh, true)
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: veh_9784 */
    RegisterCommand('veh_9784', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      _0x97841634EF7DF1D6(veh, true);
    });
    ```
- **Caveats / Limitations**:
  - Undocumented native.
- **Reference**: https://docs.fivem.net/natives/?_0x97841634EF7DF1D6

##### _0x9849DE24FCF23CCC
- **Name**: _0x9849DE24FCF23CCC
- **Scope**: Client
- **Signature**: `void _0x9849DE24FCF23CCC(Vehicle vehicle, BOOL toggle);`
- **Purpose**: Undocumented native toggling an unknown vehicle feature.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Target vehicle.
  - `toggle` (`BOOL`): Enable or disable.
  - **Returns**: `void` None.
...
- **Examples**:
  - Lua:
    ...
    --[[
        -- Type: Command
        -- Name: veh_9849
        -- Use: Demonstrates _0x9849DE24FCF23CCC
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('veh_9849', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        _0x9849DE24FCF23CCC(veh, true)
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: veh_9849 */
    RegisterCommand('veh_9849', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      _0x9849DE24FCF23CCC(veh, true);
    });
    ```
- **Caveats / Limitations**:
  - Undocumented native.
  - TODO(next-run): verify behavior.
- **Reference**: https://docs.fivem.net/natives/?_0x9849DE24FCF23CCC

##### _0x99A05839C46CE316
- **Name**: _0x99A05839C46CE316
- **Scope**: Client
- **Signature**: `void _0x99A05839C46CE316(BOOL toggle);`
- **Purpose**: Null subroutine; documented as doing nothing.
- **Parameters / Returns**:
  - `toggle` (`BOOL`): Unused.
  - **Returns**: `void` None.
- **OneSync / Networking**: None; has no effect.
- **Examples**:
  - Lua:
    ...
    --[[
        -- Type: Command
        -- Name: veh_99a0
        -- Use: Demonstrates _0x99A05839C46CE316
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('veh_99a0', function()
        _0x99A05839C46CE316(true)
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: veh_99a0 */
    RegisterCommand('veh_99a0', () => {
      _0x99A05839C46CE316(true);
    });
    ```
- **Caveats / Limitations**:
  - Documented as a no-op.
- **Reference**: https://docs.fivem.net/natives/?_0x99A05839C46CE316

##### _0x99CAD8E7AFDB60FA
- **Name**: _0x99CAD8E7AFDB60FA
- **Scope**: Client
- **Signature**: `void _0x99CAD8E7AFDB60FA(Vehicle vehicle, float p1, float p2);`
- **Purpose**: Undocumented native adjusting unknown vehicle parameters.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Target vehicle.
  - `p1` (`float`): Unknown.
  - `p2` (`float`): Unknown.
...
- **Examples**:
  - Lua:
    ...
    --[[
        -- Type: Command
        -- Name: veh_99ca
        -- Use: Demonstrates _0x99CAD8E7AFDB60FA
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('veh_99ca', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        _0x99CAD8E7AFDB60FA(veh, 0.0, 0.0)
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: veh_99ca */
    RegisterCommand('veh_99ca', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      _0x99CAD8E7AFDB60FA(veh, 0.0, 0.0);
    });
    ```
- **Caveats / Limitations**:
  - Undocumented native.
  - TODO(next-run): verify semantics.
- **Reference**: https://docs.fivem.net/natives/?_0x99CAD8E7AFDB60FA

##### _0x9BDDC73CC6A115D4
- **Name**: _0x9BDDC73CC6A115D4
- **Scope**: Client
- **Signature**: `void _0x9BDDC73CC6A115D4(Vehicle vehicle, BOOL p1, BOOL p2);`
- **Purpose**: Undocumented native toggling two unknown flags on a vehicle.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Target vehicle.
  - `p1` (`BOOL`): Unknown.
  - `p2` (`BOOL`): Unknown.
...
- **Examples**:
  - Lua:
    ...
    --[[
        -- Type: Command
        -- Name: veh_9bdd
        -- Use: Demonstrates _0x9BDDC73CC6A115D4
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('veh_9bdd', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        _0x9BDDC73CC6A115D4(veh, true, false)
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: veh_9bdd */
    RegisterCommand('veh_9bdd', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      _0x9BDDC73CC6A115D4(veh, true, false);
    });
    ```
- **Caveats / Limitations**:
  - Undocumented native.
- **Reference**: https://docs.fivem.net/natives/?_0x9BDDC73CC6A115D4

##### _0x9BECD4B9FEF3F8A6
- **Name**: _0x9BECD4B9FEF3F8A6
- **Scope**: Client
- **Signature**: `void _0x9BECD4B9FEF3F8A6(Vehicle vehicle, BOOL p1);`
- **Purpose**: Undocumented native toggling an unknown vehicle option.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Target vehicle.
  - `p1` (`BOOL`): Unknown toggle.
  - **Returns**: `void` None.
...
- **Examples**:
  - Lua:
    ...
    --[[
        -- Type: Command
        -- Name: veh_9bec
        -- Use: Demonstrates _0x9BECD4B9FEF3F8A6
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('veh_9bec', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        _0x9BECD4B9FEF3F8A6(veh, true)
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: veh_9bec */
    RegisterCommand('veh_9bec', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      _0x9BECD4B9FEF3F8A6(veh, true);
    });
    ```
- **Caveats / Limitations**:
  - Undocumented native.
  - TODO(next-run): verify semantics.
- **Reference**: https://docs.fivem.net/natives/?_0x9BECD4B9FEF3F8A6

##### _0x9D30687C57BAA0BB
- **Name**: _0x9D30687C57BAA0BB
- **Scope**: Client
- **Signature**: `void _0x9D30687C57BAA0BB(Any p0);`
- **Purpose**: Undocumented native accepting a single parameter.
- **Parameters / Returns**:
  - `p0` (`Any`): Unknown.
  - **Returns**: `void` None.
- **OneSync / Networking**: If entity-related, run on owner.
- **Examples**:
  - Lua:
    ...
    --[[
        -- Type: Command
        -- Name: veh_9d30
        -- Use: Demonstrates _0x9D30687C57BAA0BB
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('veh_9d30', function()
        _0x9D30687C57BAA0BB(0)
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: veh_9d30 */
    RegisterCommand('veh_9d30', () => {
      _0x9D30687C57BAA0BB(0);
    });
    ```
- **Caveats / Limitations**:
  - Undocumented native.
  - TODO(next-run): verify semantics.
- **Reference**: https://docs.fivem.net/natives/?_0x9D30687C57BAA0BB

##### _0x9F3F689B814F2599
- **Name**: _0x9F3F689B814F2599
- **Scope**: Client
- **Signature**: `void _0x9F3F689B814F2599(Vehicle vehicle, BOOL p1);`
- **Purpose**: Undocumented native toggling an unknown vehicle setting.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Target vehicle.
  - `p1` (`BOOL`): Unknown toggle.
  - **Returns**: `void` None.
...
- **Examples**:
  - Lua:
    ...
    --[[
        -- Type: Command
        -- Name: veh_9f3f
        -- Use: Demonstrates _0x9F3F689B814F2599
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('veh_9f3f', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        _0x9F3F689B814F2599(veh, true)
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: veh_9f3f */
    RegisterCommand('veh_9f3f', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      _0x9F3F689B814F2599(veh, true);
    });
    ```
- **Caveats / Limitations**:
  - Undocumented native.
- **Reference**: https://docs.fivem.net/natives/?_0x9F3F689B814F2599

##### _0xEF9D388F8D377F44
- **Name**: _0xEF9D388F8D377F44
- **Scope**: Client
- **Signature**: `void _0xEF9D388F8D377F44(Vehicle vehicle, BOOL p1);`
- **Purpose**: Undocumented native used in `fm_content_cargo` scripts.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Target vehicle.
  - `p1` (`BOOL`): Unknown flag.
  - **Returns**: `void` None.
...
- **Examples**:
  - Lua:
    ...
    --[[
        -- Type: Command
        -- Name: veh_ef9d
        -- Use: Demonstrates _0xEF9D388F8D377F44
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('veh_ef9d', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        _0xEF9D388F8D377F44(veh, true)
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: veh_ef9d */
    RegisterCommand('veh_ef9d', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      _0xEF9D388F8D377F44(veh, true);
    });
    ```
- **Caveats / Limitations**:
  - Used only in specific R* scripts; behavior unclear.
- **Reference**: https://docs.fivem.net/natives/?_0xEF9D388F8D377F44

##### _0xA01BC64DD4BFBBAC
- **Name**: _0xA01BC64DD4BFBBAC
- **Scope**: Client
- **Signature**: `Hash _0xA01BC64DD4BFBBAC(Vehicle vehicle, int seatIndex);`
- **Purpose**: Retrieves a hash related to a seat’s current task.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Vehicle to query.
  - `seatIndex` (`int`): Seat to inspect.
  - **Returns**: `Hash` representing the seat task.
- **OneSync / Networking**: Run on vehicle owner for up-to-date data.
- **Examples**:
  - Lua:
    ```lua
    --[[
        -- Type: Command
        -- Name: veh_a01b
        -- Use: Demonstrates _0xA01BC64DD4BFBBAC
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('veh_a01b', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        local hash = _0xA01BC64DD4BFBBAC(veh, -1)
        print(hash)
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: veh_a01b */
    RegisterCommand('veh_a01b', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      const hash = _0xA01BC64DD4BFBBAC(veh, -1);
      console.log(hash);
    });
    ```
- **Caveats / Limitations**:
  - Return hash meaning undocumented.
- **Reference**: https://docs.fivem.net/natives/?_0xA01BC64DD4BFBBAC

##### _0xA247F9EF01D8082E
- **Name**: _0xA247F9EF01D8082E
- **Scope**: Client
- **Signature**: `void _0xA247F9EF01D8082E(Any p0);`
- **Purpose**: Undocumented native with a single parameter.
- **Parameters / Returns**:
  - `p0` (`Any`): Unknown.
  - **Returns**: `void` None.
- **OneSync / Networking**: If entity-related, run on owner.
- **Examples**:
  - Lua:
    ...
    --[[
        -- Type: Command
        -- Name: veh_a247
        -- Use: Demonstrates _0xA247F9EF01D8082E
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('veh_a247', function()
        _0xA247F9EF01D8082E(0)
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: veh_a247 */
    RegisterCommand('veh_a247', () => {
      _0xA247F9EF01D8082E(0);
    });
    ```
- **Caveats / Limitations**:
  - Undocumented native.
  - TODO(next-run): verify semantics.
- **Reference**: https://docs.fivem.net/natives/?_0xA247F9EF01D8082E

##### _0xA4822F1CF23F4810
- **Name**: _0xA4822F1CF23F4810
- **Scope**: Client
- **Signature**: `BOOL _0xA4822F1CF23F4810(Vector3* outVec, Vector3* p1, Vector3* outVec1, Any p3, Any p4, Any p5, Any p6, Any p7, Any p8);`
- **Purpose**: Retrieves unknown positional data.
- **Parameters / Returns**:
  - `outVec` (`Vector3*`): Output vector.
  - `p1` (`Vector3*`): Input vector.
  - `outVec1` (`Vector3*`): Secondary output.
  - `p3` (`Any`): Unknown.
  - `p4` (`Any`): Unknown.
  - `p5` (`Any`): Unknown.
  - `p6` (`Any`): Unknown.
  ...
  - `p8` (`Any`): Unknown.
  - **Returns**: `BOOL` success flag.
- **OneSync / Networking**: Positional queries are local; not replicated.
- **Examples**:
  - Lua:
    ```lua
    --[[
        -- Type: Command
        -- Name: veh_a482
        -- Use: Demonstrates _0xA4822F1CF23F4810
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('veh_a482', function()
        local out = vector3(0.0, 0.0, 0.0)
        local inVec = vector3(0.0, 0.0, 0.0)
        local out2 = vector3(0.0, 0.0, 0.0)
        print(_0xA4822F1CF23F4810(out, inVec, out2, 0, 0, 0, 0, 0, 0))
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: veh_a482 */
    RegisterCommand('veh_a482', () => {
      const out = { x: 0.0, y: 0.0, z: 0.0 };
      const inVec = { x: 0.0, y: 0.0, z: 0.0 };
      const out2 = { x: 0.0, y: 0.0, z: 0.0 };
      console.log(_0xA4822F1CF23F4810(out, inVec, out2, 0, 0, 0, 0, 0, 0));
    });
    ```
- **Caveats / Limitations**:
  - Parameters and return meaning unclear.
  - TODO(next-run): verify semantics.
- **Reference**: https://docs.fivem.net/natives/?_0xA4822F1CF23F4810

##### _0xA4A9A4C40E615885
- **Name**: _0xA4A9A4C40E615885
- **Scope**: Client
- **Signature**: `void _0xA4A9A4C40E615885(Any p0);`
- **Purpose**: Undocumented native with a single parameter.
- **Parameters / Returns**:
  - `p0` (`Any`): Unknown.
  - **Returns**: `void` None.
...
- **Examples**:
  - Lua:
    ...
    --[[
        -- Type: Command
        -- Name: veh_a4a9
        -- Use: Demonstrates _0xA4A9A4C40E615885
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('veh_a4a9', function()
        _0xA4A9A4C40E615885(0)
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: veh_a4a9 */
    RegisterCommand('veh_a4a9', () => {
      _0xA4A9A4C40E615885(0);
    });
    ```
- **Caveats / Limitations**:
  - Undocumented native.
  - TODO(next-run): verify semantics.
- **Reference**: https://docs.fivem.net/natives/?_0xA4A9A4C40E615885

##### _0xA7DCDF4DED40A8F4
- **Name**: _0xA7DCDF4DED40A8F4
- **Scope**: Client
- **Signature**: `void _0xA7DCDF4DED40A8F4(Vehicle vehicle, BOOL p1);`
- **Purpose**: Undocumented native toggling an unknown vehicle state.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Target vehicle.
  - `p1` (`BOOL`): Unknown toggle.
...
- **Examples**:
  - Lua:
    ...
    --[[
        -- Type: Command
        -- Name: veh_a7dc
        -- Use: Demonstrates _0xA7DCDF4DED40A8F4
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('veh_a7dc', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        _0xA7DCDF4DED40A8F4(veh, true)
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: veh_a7dc */
    RegisterCommand('veh_a7dc', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      _0xA7DCDF4DED40A8F4(veh, true);
    });
    ```
- **Caveats / Limitations**:
  - Undocumented native.
- **Reference**: https://docs.fivem.net/natives/?_0xA7DCDF4DED40A8F4

##### _0xAA653AE61924B0A0
- **Name**: _0xAA653AE61924B0A0
- **Scope**: Client
- **Signature**: `void _0xAA653AE61924B0A0(Vehicle vehicle, BOOL toggle);`
- **Purpose**: Undocumented native toggling an unknown vehicle mode.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Target vehicle.
  - `toggle` (`BOOL`): Enable or disable.
  - **Returns**: `void` None.
...
- **Examples**:
  - Lua:
    ...
    --[[
        -- Type: Command
        -- Name: veh_aa65
        -- Use: Demonstrates _0xAA653AE61924B0A0
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('veh_aa65', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        _0xAA653AE61924B0A0(veh, true)
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: veh_aa65 */
    RegisterCommand('veh_aa65', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      _0xAA653AE61924B0A0(veh, true);
    });
    ```
- **Caveats / Limitations**:
  - Undocumented native.
- **Reference**: https://docs.fivem.net/natives/?_0xAA653AE61924B0A0

##### _0xAB04325045427AAE
- **Name**: _0xAB04325045427AAE
- **Scope**: Client
- **Signature**: `void _0xAB04325045427AAE(Vehicle vehicle, BOOL p1);`
- **Purpose**: Undocumented native toggling an unknown vehicle flag.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Target vehicle.
  - `p1` (`BOOL`): Unknown.
  - **Returns**: `void` None.
...
- **Examples**:
  - Lua:
    ...
    --[[
        -- Type: Command
        -- Name: veh_ab04
        -- Use: Demonstrates _0xAB04325045427AAE
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('veh_ab04', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        _0xAB04325045427AAE(veh, true)
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: veh_ab04 */
    RegisterCommand('veh_ab04', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      _0xAB04325045427AAE(veh, true);
    });
    ```
- **Caveats / Limitations**:
  - Undocumented native.
- **Reference**: https://docs.fivem.net/natives/?_0xAB04325045427AAE

##### _0xAB31EF4DE6800CE9
- **Name**: _0xAB31EF4DE6800CE9
- **Scope**: Client
- **Signature**: `void _0xAB31EF4DE6800CE9(Any p0, Any p1);`
- **Purpose**: Undocumented native with two unknown parameters.
- **Parameters / Returns**:
  - `p0` (`Any`): Unknown.
  - `p1` (`Any`): Unknown.
  - **Returns**: `void` None.
- **OneSync / Networking**: Entity effects require ownership; run on relevant owner.
- **Examples**:
  - Lua:
    ```lua
    --[[
        -- Type: Command
        -- Name: veh_ab31
        -- Use: Demonstrates _0xAB31EF4DE6800CE9
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('veh_ab31', function()
        _0xAB31EF4DE6800CE9(0, 0)
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: veh_ab31 */
    RegisterCommand('veh_ab31', () => {
      _0xAB31EF4DE6800CE9(0, 0);
    });
    ```
- **Caveats / Limitations**:
  - Undocumented native.
  - TODO(next-run): verify semantics.
- **Reference**: https://docs.fivem.net/natives/?_0xAB31EF4DE6800CE9

##### _0xAF60E6A2936F982A
- **Name**: _0xAF60E6A2936F982A
- **Scope**: Client
- **Signature**: `void _0xAF60E6A2936F982A(Any p0, Any p1);`
- **Purpose**: Undocumented native with 2 unknown parameter(s).
- **Parameters / Returns**:
  - `p0` (`Any`): Unknown.
  - `p1` (`Any`): Unknown.
  - **Returns**: `void` None.
- **OneSync / Networking**: Run on entity owner for replication if entity-related.
- **Examples**:
  - Lua:
    ```lua
    --[[
        -- Type: Command
        -- Name: veh_af60
        -- Use: Demonstrates _0xAF60E6A2936F982A
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('veh_af60', function()
        _0xAF60E6A2936F982A(0, 0)
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: veh_af60 */
    RegisterCommand('veh_af60', () => {
      _0xAF60E6A2936F982A(0, 0);
    });
    ```
- **Caveats / Limitations**:
  - Undocumented native.
- **Reference**: https://docs.fivem.net/natives/?_0xAF60E6A2936F982A

##### _0xB2E0C0D6922D31F2
- **Name**: _0xB2E0C0D6922D31F2
- **Scope**: Client
- **Signature**: `void _0xB2E0C0D6922D31F2(Vehicle vehicle, BOOL toggle);`
- **Purpose**: Undocumented native toggling an unknown vehicle flag.
- **Parameters / Returns**:
...
- **Examples**:
  - Lua:
    ...
    --[[
        -- Type: Command
        -- Name: veh_b2e0
        -- Use: Demonstrates _0xB2E0C0D6922D31F2
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('veh_b2e0', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        _0xB2E0C0D6922D31F2(veh, 0)
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: veh_b2e0 */
    RegisterCommand('veh_b2e0', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      _0xB2E0C0D6922D31F2(veh, 0);
    });
    ```
- **Caveats / Limitations**:
  - Undocumented native.
- **Reference**: https://docs.fivem.net/natives/?_0xB2E0C0D6922D31F2

##### _0xB68CFAF83A02768D
- **Name**: _0xB68CFAF83A02768D
- **Scope**: Client
- **Signature**: `void _0xB68CFAF83A02768D(Vehicle vehicle, BOOL toggle);`
- **Purpose**: Undocumented native toggling an unknown vehicle flag.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Unknown.
  - `toggle` (`BOOL`): Unknown.
...
- **Examples**:
  - Lua:
    ...
    --[[
        -- Type: Command
        -- Name: veh_b68c
        -- Use: Demonstrates _0xB68CFAF83A02768D
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('veh_b68c', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        _0xB68CFAF83A02768D(veh, 0)
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: veh_b68c */
    RegisterCommand('veh_b68c', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      _0xB68CFAF83A02768D(veh, 0);
    });
    ```
- **Caveats / Limitations**:
  - Undocumented native.
- **Reference**: https://docs.fivem.net/natives/?_0xB68CFAF83A02768D

##### _0xB9562064627FF9DB
- **Name**: _0xB9562064627FF9DB
- **Scope**: Client
- **Signature**: `void _0xB9562064627FF9DB(Any p0, Any p1);`
- **Purpose**: Undocumented native with 2 unknown parameter(s).
- **Parameters / Returns**:
  - `p0` (`Any`): Unknown.
  - `p1` (`Any`): Unknown.
...
- **Examples**:
  - Lua:
    ...
    --[[
        -- Type: Command
        -- Name: veh_b956
        -- Use: Demonstrates _0xB9562064627FF9DB
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('veh_b956', function()
        _0xB9562064627FF9DB(0, 0)
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: veh_b956 */
    RegisterCommand('veh_b956', () => {
      _0xB9562064627FF9DB(0, 0);
    });
    ```
- **Caveats / Limitations**:
  - Undocumented native.
- **Reference**: https://docs.fivem.net/natives/?_0xB9562064627FF9DB

##### _0xBB2333BB87DDD87F
- **Name**: _0xBB2333BB87DDD87F
- **Scope**: Client
- **Signature**: `void _0xBB2333BB87DDD87F(Any p0, Any p1);`
- **Purpose**: Undocumented native with 2 unknown parameter(s).
- **Parameters / Returns**:
  - `p0` (`Any`): Unknown.
  - `p1` (`Any`): Unknown.
...
- **Examples**:
  - Lua:
    ...
    --[[
        -- Type: Command
        -- Name: veh_bb23
        -- Use: Demonstrates _0xBB2333BB87DDD87F
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('veh_bb23', function()
        _0xBB2333BB87DDD87F(0, 0)
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: veh_bb23 */
    RegisterCommand('veh_bb23', () => {
      _0xBB2333BB87DDD87F(0, 0);
    });
    ```
- **Caveats / Limitations**:
  - Undocumented native.
- **Reference**: https://docs.fivem.net/natives/?_0xBB2333BB87DDD87F

##### _0xBE5C1255A1830FF5
- **Name**: _0xBE5C1255A1830FF5
- **Scope**: Client
- **Signature**: `void _0xBE5C1255A1830FF5(Vehicle vehicle, BOOL toggle);`
- **Purpose**: Undocumented native toggling an unknown vehicle flag.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Unknown.
  - `toggle` (`BOOL`): Unknown.
...
- **Examples**:
  - Lua:
    ...
    --[[
        -- Type: Command
        -- Name: veh_be5c
        -- Use: Demonstrates _0xBE5C1255A1830FF5
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('veh_be5c', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        _0xBE5C1255A1830FF5(veh, 0)
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: veh_be5c */
    RegisterCommand('veh_be5c', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      _0xBE5C1255A1830FF5(veh, 0);
    });
    ```
- **Caveats / Limitations**:
  - Undocumented native.
- **Reference**: https://docs.fivem.net/natives/?_0xBE5C1255A1830FF5

##### _0xC0ED6438E6D39BA8
- **Name**: _0xC0ED6438E6D39BA8
- **Scope**: Client
- **Signature**: `void _0xC0ED6438E6D39BA8(Any p0, Any p1, Any p2);`
- **Purpose**: Undocumented native with 3 unknown parameter(s).
- **Parameters / Returns**:
  - `p0` (`Any`): Unknown.
  - `p1` (`Any`): Unknown.
  - `p2` (`Any`): Unknown.
...
- **Examples**:
  - Lua:
    ...
    --[[
        -- Type: Command
        -- Name: veh_c0ed
        -- Use: Demonstrates _0xC0ED6438E6D39BA8
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('veh_c0ed', function()
        _0xC0ED6438E6D39BA8(0, 0, 0)
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: veh_c0ed */
    RegisterCommand('veh_c0ed', () => {
      _0xC0ED6438E6D39BA8(0, 0, 0);
    });
    ```
- **Caveats / Limitations**:
  - Undocumented native.
- **Reference**: https://docs.fivem.net/natives/?_0xC0ED6438E6D39BA8

##### _0xC361AA040D6637A8
- **Name**: _0xC361AA040D6637A8
- **Scope**: Client
- **Signature**: `void _0xC361AA040D6637A8(Vehicle vehicle, BOOL p1);`
- **Purpose**: Undocumented native toggling an unknown vehicle flag.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Unknown.
  - `p1` (`BOOL`): Unknown.
...
- **Examples**:
  - Lua:
    ...
    --[[
        -- Type: Command
        -- Name: veh_c361
        -- Use: Demonstrates _0xC361AA040D6637A8
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('veh_c361', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        _0xC361AA040D6637A8(veh, 0)
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: veh_c361 */
    RegisterCommand('veh_c361', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      _0xC361AA040D6637A8(veh, 0);
    });
    ```
- **Caveats / Limitations**:
  - Undocumented native.
- **Reference**: https://docs.fivem.net/natives/?_0xC361AA040D6637A8

##### _0xC4B3347BD68BD609
- **Name**: _0xC4B3347BD68BD609
- **Scope**: Client
- **Signature**: `void _0xC4B3347BD68BD609(Any p0);`
- **Purpose**: Undocumented native with 1 unknown parameter(s).
- **Parameters / Returns**:
  - `p0` (`Any`): Unknown.
  - **Returns**: `void` None.
- **OneSync / Networking**: Run on entity owner for replication if entity-related.
- **Examples**:
  - Lua:
    ...
    --[[
        -- Type: Command
        -- Name: veh_c4b3
        -- Use: Demonstrates _0xC4B3347BD68BD609
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('veh_c4b3', function()
        _0xC4B3347BD68BD609(0)
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: veh_c4b3 */
    RegisterCommand('veh_c4b3', () => {
      _0xC4B3347BD68BD609(0);
    });
    ```
- **Caveats / Limitations**:
  - Undocumented native.
- **Reference**: https://docs.fivem.net/natives/?_0xC4B3347BD68BD609

##### _0xC50CE861B55EAB8B
- **Name**: _0xC50CE861B55EAB8B
- **Scope**: Client
- **Signature**: `void _0xC50CE861B55EAB8B(Vehicle vehicle, BOOL p1);`
- **Purpose**: Undocumented native toggling an unknown vehicle flag.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Unknown.
  - `p1` (`BOOL`): Unknown.
...
- **Examples**:
  - Lua:
    ...
    --[[
        -- Type: Command
        -- Name: veh_c50c
        -- Use: Demonstrates _0xC50CE861B55EAB8B
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('veh_c50c', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        _0xC50CE861B55EAB8B(veh, 0)
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: veh_c50c */
    RegisterCommand('veh_c50c', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      _0xC50CE861B55EAB8B(veh, 0);
    });
    ```
- **Caveats / Limitations**:
  - Undocumented native.
- **Reference**: https://docs.fivem.net/natives/?_0xC50CE861B55EAB8B

##### _0xCF9159024555488C
- **Name**: _0xCF9159024555488C
- **Scope**: Client
- **Signature**: `void _0xCF9159024555488C(Any p0);`
- **Purpose**: Undocumented native with 1 unknown parameter(s).
- **Parameters / Returns**:
  - `p0` (`Any`): Unknown.
  - **Returns**: `void` None.
...
- **Examples**:
  - Lua:
    ...
    --[[
        -- Type: Command
        -- Name: veh_cf91
        -- Use: Demonstrates _0xCF9159024555488C
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('veh_cf91', function()
        _0xCF9159024555488C(0)
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: veh_cf91 */
    RegisterCommand('veh_cf91', () => {
      _0xCF9159024555488C(0);
    });
    ```
- **Caveats / Limitations**:
  - Undocumented native.
- **Reference**: https://docs.fivem.net/natives/?_0xCF9159024555488C

##### _0xCFD778E7904C255E
- **Name**: _0xCFD778E7904C255E
- **Scope**: Client
- **Signature**: `void _0xCFD778E7904C255E(Vehicle vehicle);`
- **Purpose**: Undocumented native affecting a vehicle with unknown effect.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Unknown.
  - **Returns**: `void` None.
- **OneSync / Networking**: Run on entity owner for replication if entity-related.
- **Examples**:
  - Lua:
    ...
    --[[
        -- Type: Command
        -- Name: veh_cfd7
        -- Use: Demonstrates _0xCFD778E7904C255E
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('veh_cfd7', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        _0xCFD778E7904C255E(veh)
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: veh_cfd7 */
    RegisterCommand('veh_cfd7', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      _0xCFD778E7904C255E(veh);
    });
    ```
- **Caveats / Limitations**:
  - Undocumented native.
- **Reference**: https://docs.fivem.net/natives/?_0xCFD778E7904C255E

##### _0xD3301660A57C9272
- **Name**: _0xD3301660A57C9272
- **Scope**: Client
- **Signature**: `void _0xD3301660A57C9272(Any p0);`
- **Purpose**: Undocumented native with 1 unknown parameter(s).
- **Parameters / Returns**:
  - `p0` (`Any`): Unknown.
  - **Returns**: `void` None.
...
- **Examples**:
  - Lua:
    ...
    --[[
        -- Type: Command
        -- Name: veh_d330
        -- Use: Demonstrates _0xD3301660A57C9272
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('veh_d330', function()
        _0xD3301660A57C9272(0)
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: veh_d330 */
    RegisterCommand('veh_d330', () => {
      _0xD3301660A57C9272(0);
    });
    ```
- **Caveats / Limitations**:
  - Undocumented native.
- **Reference**: https://docs.fivem.net/natives/?_0xD3301660A57C9272

##### _0xD3E51C0AB8C26EEE
- **Name**: _0xD3E51C0AB8C26EEE
- **Scope**: Client
- **Signature**: `Any _0xD3E51C0AB8C26EEE(Any p0, Any p1);`
- **Purpose**: Undocumented native with 2 unknown parameter(s).
- **Parameters / Returns**:
  - `p0` (`Any`): Unknown.
  - `p1` (`Any`): Unknown.
...
- **Examples**:
  - Lua:
    ...
    --[[
        -- Type: Command
        -- Name: veh_d3e5
        -- Use: Demonstrates _0xD3E51C0AB8C26EEE
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('veh_d3e5', function()
        _0xD3E51C0AB8C26EEE(0, 0)
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: veh_d3e5 */
    RegisterCommand('veh_d3e5', () => {
      _0xD3E51C0AB8C26EEE(0, 0);
    });
    ```
- **Caveats / Limitations**:
  - Undocumented native.
- **Reference**: https://docs.fivem.net/natives/?_0xD3E51C0AB8C26EEE

##### _0xD4196117AF7BB974
- **Name**: _0xD4196117AF7BB974
- **Scope**: Client
- **Signature**: `Any _0xD4196117AF7BB974(Any p0, Any p1);`
- **Purpose**: Undocumented native with 2 unknown parameter(s).
- **Parameters / Returns**:
  - `p0` (`Any`): Unknown.
  - `p1` (`Any`): Unknown.
...
- **Examples**:
  - Lua:
    ...
    --[[
        -- Type: Command
        -- Name: veh_d419
        -- Use: Demonstrates _0xD4196117AF7BB974
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('veh_d419', function()
        _0xD4196117AF7BB974(0, 0)
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: veh_d419 */
    RegisterCommand('veh_d419', () => {
      _0xD4196117AF7BB974(0, 0);
    });
    ```
- **Caveats / Limitations**:
  - Undocumented native.
- **Reference**: https://docs.fivem.net/natives/?_0xD4196117AF7BB974

##### _0xD565F438137F0E10
- **Name**: _0xD565F438137F0E10
- **Scope**: Client
- **Signature**: `void _0xD565F438137F0E10(Any p0, Any p1);`
- **Purpose**: Undocumented native with 2 unknown parameter(s).
- **Parameters / Returns**:
  - `p0` (`Any`): Unknown.
  - `p1` (`Any`): Unknown.
...
- **Examples**:
  - Lua:
    ...
    --[[
        -- Type: Command
        -- Name: veh_d565
        -- Use: Demonstrates _0xD565F438137F0E10
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('veh_d565', function()
        _0xD565F438137F0E10(0, 0)
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: veh_d565 */
    RegisterCommand('veh_d565', () => {
      _0xD565F438137F0E10(0, 0);
    });
    ```
- **Caveats / Limitations**:
  - Undocumented native.
- **Reference**: https://docs.fivem.net/natives/?_0xD565F438137F0E10

##### _0xDBC631F109350B8C
- **Name**: _0xDBC631F109350B8C
- **Scope**: Client
- **Signature**: `void _0xDBC631F109350B8C(Vehicle vehicle, BOOL p1);`
- **Purpose**: Undocumented native toggling an unknown vehicle flag.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Unknown.
  - `p1` (`BOOL`): Unknown.
...
- **Examples**:
  - Lua:
    ...
    --[[
        -- Type: Command
        -- Name: veh_dbc6
        -- Use: Demonstrates _0xDBC631F109350B8C
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('veh_dbc6', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        _0xDBC631F109350B8C(veh, 0)
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: veh_dbc6 */
    RegisterCommand('veh_dbc6', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      _0xDBC631F109350B8C(veh, 0);
    });
    ```
- **Caveats / Limitations**:
  - Undocumented native.
- **Reference**: https://docs.fivem.net/natives/?_0xDBC631F109350B8C

##### _0xDCE97BDF8A0EABC8
- **Name**: _0xDCE97BDF8A0EABC8
- **Scope**: Client
- **Signature**: `void _0xDCE97BDF8A0EABC8();`
...
- **Examples**:
  - Lua:
    ...
    --[[
        -- Type: Command
        -- Name: veh_dce9
        -- Use: Demonstrates _0xDCE97BDF8A0EABC8
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('veh_dce9', function()
        _0xDCE97BDF8A0EABC8()
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: veh_dce9 */
    RegisterCommand('veh_dce9', () => {
      _0xDCE97BDF8A0EABC8();
    });
    ```
- **Caveats / Limitations**:
  - Undocumented native.
- **Reference**: https://docs.fivem.net/natives/?_0xDCE97BDF8A0EABC8

##### _0xE05DD0E9707003A3
- **Name**: _0xE05DD0E9707003A3
- **Scope**: Client
- **Signature**: `void _0xE05DD0E9707003A3(Any p0, BOOL p1);`
- **Purpose**: Undocumented native with 2 unknown parameter(s).
- **Parameters / Returns**:
  - `p0` (`Any`): Unknown.
  - `p1` (`BOOL`): Unknown.
...
- **Examples**:
  - Lua:
    ...
    --[[
        -- Type: Command
        -- Name: veh_e05d
        -- Use: Demonstrates _0xE05DD0E9707003A3
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('veh_e05d', function()
        _0xE05DD0E9707003A3(0, 0)
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: veh_e05d */
    RegisterCommand('veh_e05d', () => {
      _0xE05DD0E9707003A3(0, 0);
    });
    ```
- **Caveats / Limitations**:
  - Undocumented native.
- **Reference**: https://docs.fivem.net/natives/?_0xE05DD0E9707003A3

##### _0xE2F53F172B45EDE1
- **Name**: _0xE2F53F172B45EDE1
- **Scope**: Client
- **Signature**: `void _0xE2F53F172B45EDE1();`
- **Purpose**: Undocumented native with no parameters.
- **Parameters / Returns**:
  - **Returns**: `void` None.
- **OneSync / Networking**: Run on entity owner for replication if entity-related.
- **Examples**:
  - Lua:
    ...
    --[[
        -- Type: Command
        -- Name: veh_e2f5
        -- Use: Demonstrates _0xE2F53F172B45EDE1
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('veh_e2f5', function()
        _0xE2F53F172B45EDE1()
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: veh_e2f5 */
    RegisterCommand('veh_e2f5', () => {
      _0xE2F53F172B45EDE1();
    });
    ```
- **Caveats / Limitations**:
  - Undocumented native.
- **Reference**: https://docs.fivem.net/natives/?_0xE2F53F172B45EDE1

##### _0xE5810AC70602F2F5
- **Name**: _0xE5810AC70602F2F5
- **Scope**: Client
- **Signature**: `void _0xE5810AC70602F2F5(Vehicle vehicle, float p1);`
- **Purpose**: Undocumented native modifying a vehicle with unspecified parameters.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Unknown.
  - `p1` (`float`): Unknown.
  - **Returns**: `void` None.
...
- **Examples**:
  - Lua:
    ...
    --[[
        -- Type: Command
        -- Name: veh_e581
        -- Use: Demonstrates _0xE5810AC70602F2F5
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('veh_e581', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        _0xE5810AC70602F2F5(veh, 0)
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: veh_e581 */
    RegisterCommand('veh_e581', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      _0xE5810AC70602F2F5(veh, 0);
    });
    ```
- **Caveats / Limitations**:
  - Undocumented native.
- **Reference**: https://docs.fivem.net/natives/?_0xE5810AC70602F2F5

##### _0xE851E480B814D4BA
- **Name**: _0xE851E480B814D4BA
- **Scope**: Client
- **Signature**: `void _0xE851E480B814D4BA(Vehicle vehicle, BOOL p1);`
- **Purpose**: Undocumented native toggling an unknown vehicle flag.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Unknown.
  - `p1` (`BOOL`): Unknown.
...
- **Examples**:
  - Lua:
    ...
    --[[
        -- Type: Command
        -- Name: veh_e851
        -- Use: Demonstrates _0xE851E480B814D4BA
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('veh_e851', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        _0xE851E480B814D4BA(veh, 0)
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: veh_e851 */
    RegisterCommand('veh_e851', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      _0xE851E480B814D4BA(veh, 0);
    });
    ```
- **Caveats / Limitations**:
  - Undocumented native.
- **Reference**: https://docs.fivem.net/natives/?_0xE851E480B814D4BA

##### _0xE8718FAF591FD224
- **Name**: _0xE8718FAF591FD224
- **Scope**: Client
- **Signature**: `BOOL _0xE8718FAF591FD224(Vehicle vehicle);`
- **Purpose**: Undocumented native affecting a vehicle with unknown effect.
- **Parameters / Returns**:
  - `vehicle` (`Vehicle`): Unknown.
  - **Returns**: `BOOL` value.
...
- **Examples**:
  - Lua:
    ...
    --[[
        -- Type: Command
        -- Name: veh_e871
        -- Use: Demonstrates _0xE8718FAF591FD224
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('veh_e871', function()
        local veh = GetVehiclePedIsIn(PlayerPedId(), false)
        _0xE8718FAF591FD224(veh)
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: veh_e871 */
    RegisterCommand('veh_e871', () => {
      const veh = GetVehiclePedIsIn(PlayerPedId(), false);
      _0xE8718FAF591FD224(veh);
    });
    ```
- **Caveats / Limitations**:
  - Undocumented native.
- **Reference**: https://docs.fivem.net/natives/?_0xE8718FAF591FD224

##### _0xED5EDE9E676643C9
- **Name**: _0xED5EDE9E676643C9
- **Scope**: Client
- **Signature**: `void _0xED5EDE9E676643C9(Any p0, Any p1);`
- **Purpose**: Undocumented native with 2 unknown parameter(s).
- **Parameters / Returns**:
  - `p0` (`Any`): Unknown.
  - `p1` (`Any`): Unknown.
...
- **Examples**:
  - Lua:
    ...
    --[[
        -- Type: Command
        -- Name: veh_ed5e
        -- Use: Demonstrates _0xED5EDE9E676643C9
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('veh_ed5e', function()
        _0xED5EDE9E676643C9(0, 0)
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: veh_ed5e */
    RegisterCommand('veh_ed5e', () => {
      _0xED5EDE9E676643C9(0, 0);
    });
    ```
- **Caveats / Limitations**:
  - Undocumented native.
- **Reference**: https://docs.fivem.net/natives/?_0xED5EDE9E676643C9

##### _0xF051D9BFB6BA39C0
- **Name**: _0xF051D9BFB6BA39C0
- **Scope**: Client
- **Signature**: `void _0xF051D9BFB6BA39C0(Any p0);`
- **Purpose**: Undocumented native with 1 unknown parameter(s).
- **Parameters / Returns**:
  - `p0` (`Any`): Unknown.
  - **Returns**: `void` None.
...
- **Examples**:
  - Lua:
    ...
    --[[
        -- Type: Command
        -- Name: veh_f051
        -- Use: Demonstrates _0xF051D9BFB6BA39C0
        -- Created: 2025-09-12
        -- By: VSSVSSN
    --]]
    RegisterCommand('veh_f051', function()
        _0xF051D9BFB6BA39C0(0)
    end)
    ```
  - JavaScript:
    ```javascript
    /* Command: veh_f051 */
    RegisterCommand('veh_f051', () => {
      _0xF051D9BFB6BA39C0(0);
    });
    ```
- **Caveats / Limitations**:
