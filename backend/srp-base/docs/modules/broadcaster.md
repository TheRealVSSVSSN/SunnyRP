...
Attempts to assign the `broadcaster` job to the specified character.
...
If the character ID is missing, a `INVALID_INPUT` error is returned.

The server counts current assignments of the `broadcaster` job via
`jobsRepository.countCharactersForJob('broadcaster')`.  If the count
...
calls `jobsRepository.assignJob(characterId, job.id)` to assign the
...
* The module does not automatically set characters on duty; it simply
