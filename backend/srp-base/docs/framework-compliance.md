...
| **Service and data layers** | Domain logic is encapsulated in repository modules (e.g. `mechanicRepository.js`, `doorsRepository.js`) and service files.  Routes remain thin controllers that perform validation and call repository functions. |
...
| **marked-bills module** | Dirty money endpoints follow the established layered pattern with authentication, idempotency, WebSocket/webhook events. |
