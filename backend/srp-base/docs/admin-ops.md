...
- Ensure the `action_bar_slots` table exists for character quick action slots.
...
- Ensure the `character_peds` table exists; `peds-health-regen` scheduler regenerates health and emits WebSocket events.
