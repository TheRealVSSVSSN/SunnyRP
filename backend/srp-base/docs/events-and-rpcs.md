...
| np-dances | Client lists dance animations for /dance command | `GET /v1/dances/animations`, `POST /v1/dances/animations`, `DELETE /v1/dances/animations/{id}` → `dances.animationAdded`, `dances.animationRemoved`, `dances.animationExpired` |
| np-dirtymoney | `np-dirtymoney:attemptDirtyMoneyDrops`, `np-dirtymoney:alterDirtyMoney`, `np-dirtymoney:moneyPickup` | `GET /v1/characters/{characterId}/marked-bills`, `POST /v1/characters/{characterId}/marked-bills:alter`, `POST /v1/characters/{characterId}/marked-bills:pickup` |
